...
13. Converge human-maintained agent text fields: remove `Project Scope` / `Human Notes`, rename `Manual Guidance` to canonical `Guidance`, and define it as the shared human-authored intent surface for agent/supervisor/subconscious while keeping `CLAUDE.md` as workflow/behavior contract.
