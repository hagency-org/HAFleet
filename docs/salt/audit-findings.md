...
- [Critical] [backend-v2.js](/Users/kamico/hafleet/backend-v2.js:8837) Anonymous group message reads can advance another agent's group cursor.
...
- [Critical] [backend-v2.js](/Users/kamico/hafleet/backend-v2.js:8636) Message details are publicly readable.
...
- [Critical] [backend-v2.js](/Users/kamico/hafleet/backend-v2.js:166) Agent-token auth is fail-open by default.
  Impact: A local or network caller can impersonate managed agents when token mode is `audit` or no token exists.
  Evidence: `HAFLEET_AGENT_TOKEN_MODE` defaults to audit and missing expected tokens are treated as allowed.
  Fix: Make managed-agent core writes fail closed in production; keep audit/off only for explicit development or tests.

- [High] [backend-v2.js](/Users/kamico/hafleet/backend-v2.js:8572) Offline group mentions can be permanently hidden from inbox.
...
- [High] [backend-v2.js](/Users/kamico/hafleet/backend-v2.js:3437) Task graph result hooks can be spoofed by non-assignees.
...
- [Medium] [backend-v2.js](/Users/kamico/hafleet/backend-v2.js:3193) Task ownership has multiple truths.
...
- [Medium] [lib/notification-router.js](/Users/kamico/hafleet/lib/notification-router.js:78) Aggregated alert cooldown can be persisted before notification flush.
...
- [Low] [lib/agent-home-v1.js](/Users/kamico/hafleet/lib/agent-home-v1.js:5) Relative paths are accepted after normalization.
...
- [High] [lib/push-relay-core.js](/Users/kamico/hafleet/lib/push-relay-core.js:495) MCP presence detection is Linux-specific.
...
- [High] [remote/bin/hafleet](/Users/kamico/hafleet/remote/bin/hafleet:67) Remote CLI advertises missing commands.
...
- [High] [bin/hafleet-up](/Users/kamico/hafleet/bin/hafleet-up:1749) Codex MCP launch auth may be incomplete for non-v1 agents.
...
- [Medium] [lib/mcp-server-core.js](/Users/kamico/hafleet/lib/mcp-server-core.js:73) MCP media cache writes under the current working directory.
  ...
  Fix: Use `HAFLEET_AGENT_STATE_DIR/tmp/mcp-media-cache` first, then `HAFLEET_RUNTIME_DIR/data/mcp-media-cache`.

- [Medium] [bin/hafleet-down](/Users/kamico/hafleet/bin/hafleet-down:426) Forced local shutdown can be blocked by backend unavailability.
...
- [Medium] [remote/bin/hafleet-up](/Users/kamico/hafleet/remote/bin/hafleet-up:1680) Remote launch script has drifted from root launch behavior.
...
- [Medium] [bin/hafleet-service](/Users/kamico/hafleet/bin/hafleet-service:87) Service CLI assumes bash 4 features.
...
- [Low] [lib/push-relay-core.js](/Users/kamico/hafleet/lib/push-relay-core.js:575) Tmux injection sequence is over-defensive.
...
- [High] [server.js](/Users/kamico/hafleet/server.js:32) Dashboard must remain a local/operator surface, not a public privileged proxy.
  ...
  Evidence: RLP3-B1 added a local-only or `HAFLEET_DASHBOARD_TOKEN` gate for mutating APIs, but dashboard browser login/session policy is still not defined.
...
- [High] [bridge-matrix.js](/Users/kamico/hafleet/bridge-matrix.js:66) Matrix trust and command ACL defaults are fail-open.
...
- [Medium] [backend-v2.js](/Users/kamico/hafleet/backend-v2.js:14) Supervisor lifecycle is imported and initialized by the backend.
...
- [Medium] [scripts/write-supervisor-state.js](/Users/kamico/hafleet/scripts/write-supervisor-state.js:27) Supervisor state CLI advertises a registration path it does not implement.
...
- [Medium] [lib/supervisor-action-engine.js](/Users/kamico/hafleet/lib/supervisor-action-engine.js:76) Supervisor escalation target is hard-coded.
...
- [Medium] [scripts/build-remote-package.sh](/Users/kamico/hafleet/scripts/build-remote-package.sh:56) Remote package is out of sync.
...
- [Medium] [backend-v2.js](/Users/kamico/hafleet/backend-v2.js:6909) Subconscious upstream/runtime endpoints have weaker per-agent auth than event ingest.
  ...
  Fix: Reuse `HAFLEET_SUBCONSCIOUS_EVENT_TOKEN` or `requireAgentToken(name)` across all kernel-facing subconscious writes.

- [High] [scripts/audit-deps.sh](/Users/kamico/hafleet/scripts/audit-deps.sh:34) Dependency audit currently fails and is not enforced by CI/CD.
...
- [High] [.github/workflows/ci.yml](/Users/kamico/hafleet/.github/workflows/ci.yml:19) CI does not check remote sync.
...
- [Medium] [package.json](/Users/kamico/hafleet/package.json:25) Local checkout is missing dev test dependencies.
...
- [Medium] [.env.example](/Users/kamico/hafleet/.env.example:47) Environment example documents unsupported or misleading modes.
...
- [High] [backend-v2.js](/Users/kamico/hafleet/backend-v2.js:2643) Production data files lack schema/version/migration contracts.
...
- [Medium] [backend-v2.js](/Users/kamico/hafleet/backend-v2.js:269) Backend startup can mutate data during read/load.
...
- [Medium] [tests/helpers/backend-test-runtime.js](/Users/kamico/hafleet/tests/helpers/backend-test-runtime.js:21) Test helper has stale state filename and env cleanup gaps.
