...
- Completed a non-deploy idle observability batch while CD-A/CD-B deploy behavior remains operator-gated. Backend runtime activity now preserves `activeNow=null` as unknown instead of idle, and `agentchat cli status` plus `agent-ls` show unknown activity without the Bash 3.2 empty-header crash.
...
- Implemented the safe `agent-ls` slice of RLP6-B. `agent-ls --all` now discovers v1 manifests through the same absolute home/runtime/legacy roots as `lib/agent-home-v1.js`, with fake tmux/curl CLI tests and mirrored remote script parity. `agent-down` shutdown behavior was left unchanged pending separate approval.
- Implemented RLP3-B1 dashboard mutation boundary. `server.js` now allows mutating `/api` dashboard routes only from loopback callers or requests carrying `AGENT_CHAT_DASHBOARD_TOKEN`, while read-only dashboard behavior remains compatible. Backend queue bridge calls and queue/reminder CLI helpers pass the optional token when configured; full web-auth remains decision-gated.
