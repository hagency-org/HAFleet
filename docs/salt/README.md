...
- Phase 2-7 resumption is staged in `15`; RLP4-A MCP media cache relocation, RLP2-A runtime observation provenance, RLP2-B custom local server ID/local server record, RLP6-A profile-scoped help/docs, RLP3-A auth readiness diagnostics, and RLP3-A2 server credential boundary diagnostics are implemented, with the next safe batches still approval-gated.
