...
| Input | Meaning |
| --- | --- |
| `AGENT_CHAT_API` | Backend base URL. |
| `AGENT_CHAT_SERVER` | Explicit runtime host id. |
| Host credential | Credential allowed to heartbeat/report runtime for this host. |
| `AGENTCHAT_HOMEDIR` or derived home root | Root used to resolve local agent homes and state dirs. |
...
| Variable | Contract |
| --- | --- |
| `AGENT_CHAT_RUNTIME_DIR` | Backend runtime/data root. |
| `AGENT_CHAT_SERVER` | Explicit id for the central-local runtime host. Dev may use `local`. |
| `API_TOKEN` | Operator/backend bearer; production required. |
| `AGENTCHAT_AGENT_TOKEN_MODE` | Production target: `hard`. |
...
| --- | --- |
| `AGENT_CHAT_API` | Backend base URL. |
| `AGENT_CHAT_SERVER` | Explicit host id; no hostname/local fallback in production. |
| Host credential | Initially `API_TOKEN`, later `AGENTCHAT_SERVER_TOKEN` or equivalent. |
| `AGENTCHAT_HOMEDIR` | Optional override; otherwise derived from runtime contract. |
...
| `AGENT_CHAT_API` | Backend base URL. |
| `AGENT_CHAT_SERVER` | Host id assigned by launcher/manifest. |
| `AGENTCHAT_AGENT_STATE_DIR` | Derived state dir containing `agent-token` and `mcp-server.pid`. |

### Derived Paths

These should be derived from a common resolver, not hand-assembled per component:

- `homeRoot = AGENTCHAT_HOMEDIR || AGENT_CHAT_RUNTIME_DIR/homes || ~/.agentchat`
...
Remote `agentchat` should dispatch only commands included in that package. If a command is central-only, the remote CLI should say so clearly instead of dispatching to a missing file.
...
| --- | --- |
| `agentchat up` | Start a tmux agent on the current runtime host and register it to backend. |
| `agentchat down` | Stop a tmux agent on the current runtime host; backend marking is best effort. |
| `agentchat ls` | Show current host sessions plus backend registry context; not a global truth dump. |
| `agentchat service` | Control services for an explicit profile; no `all` default on remote. |
| `agentchat update` | Remote package update only, or renamed to make that scope explicit. |
| `agentchat verify-remote` | Verify one runtime host against central backend. |
| `agentchat check-mcp` | Profile-specific; only advertised where packaged. |
...
- Do not hand-edit `bin/agent-up` or `remote/bin/agent-up`; first extract launch contracts and tests.
