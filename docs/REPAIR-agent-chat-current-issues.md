...
Scope: **仅 agent-chat 自身**（启动、registry、调度、Matrix bridge、可观测性）

---

## 1. 目的

把当前线上/联调里反复出现的 agent-chat 故障，收敛成一份可执行的修复清单：
...
本文不讨论外部系统（如 OpenFab / agentd）应如何演进；只回答：**agent-chat 现在哪里坏了、怎么修。**

---

## 2. 现状摘要

agent-chat 已具备完整本地多 agent 控制面：
...
| `bridge-matrix.js` | Matrix ↔ agent-chat |
| `bin/agent-up*` / `agent-down` / `agent-ls` | 启动与运维 CLI |
...
### P-09 OpenFab signoff 语义被误用为强制门禁（agent-chat 侧）

**现象**
- 执行完成后，系统/调用方把“未 signoff”当成“未交付”
- agent-chat 路径混入认证语义，排障困难
...
**根因**
agent-chat 作为执行层，没有把状态拆清：

- `delivered/executed`（执行完成）
- `certified/signed`（外部认证，可选）

**修复方向**
1. agent-chat 任务状态只保证执行域：
...
- backend groups
- project 管理（`agent-project` / agent home projects）
...
1. Matrix 只发：
   ...
   - 状态摘要
   - 结果链接 / task id
2. 完整 transcript、diff、日志、产物进 agent-chat storage（消息库/对象存储/agent outputs）
3. 长内容默认转存储 + 摘要回房
...
agent-chat 当前不是“缺功能”，而是 **关键状态机没闭环**：
...
| agent 生命周期 | `lib/agent-state.js`, `bin/agent-up`, `backend-v2.js` registry |
