...
export function createTaskStore({ initialData, save }) {
  ...
  function snapshotTasks() {
    return new Map([...tasks.entries()].map(([id, task]) => [id, cloneTask(task)]));
  }

  function restoreTasks(snapshot) {
    tasks.clear();
    for (const [id, task] of snapshot.entries()) {
      tasks.set(id, task);
    }
  }
  ...
  function persist() {
    let ok;
    try {
      ok = save([...tasks.values()]);
    } catch {
      throw createTaskError('persistence_failed', 'task persistence failed');
    }
    if (ok === false) {
      throw createTaskError('persistence_failed', 'task persistence failed');
    }
  ...
  function updateTask(id, patch) {
    ...
    return commitMutation(() => {
      ...
      return { value: task, persist: changed };
    });
  }
  ...
  function addComment(id, comment) {
    ...
    return commitMutation(() => {
      ...
      if (task.comments.length >= 100) throw createTaskError('limit_exceeded', 'max 100 comments per task');
