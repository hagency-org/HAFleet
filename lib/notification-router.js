...
export class NotificationRouter {
  ...
  _getState(family) {
    ...
    s = {
      ...
      pendingCooldownOptions: hasAggregation ? new Map() : null,
      flushTimer: null,
      burstCount: new Map(),
      cbFails: 0,
      cbOpenUntil: 0,
  ...
  emit(family, payload, options = {}) {
    ...
    if (state.buffer !== null) {
      ...
      if (state.pendingCooldownOptions) {
        state.pendingCooldownOptions.set(key, {
          skipPersistedWrite: options.skipPersistedWrite === true,
        });
      }
    ...
    // Immediate dispatch
    const dispatched = this._dispatch(family, cfg, payload, options);
    if (dispatched) this._commitCooldown(cfg, state, key, now, options);
  ...
  flush(family) {
    ...
    const cooldownOptions = new Map(state.pendingCooldownOptions || []);
    state.buffer.clear();
    if (state.pendingCooldownOptions) state.pendingCooldownOptions.clear();
    state.burstCount.clear();
    const aggregated = cfg.aggregateFn
      ? cfg.aggregateFn(entries)
      : { entries: [...entries.values()] };
    if (aggregated) {
      const dispatched = this._dispatch(family, cfg, aggregated, { aggregated: true });
      if (dispatched) {
        const now = Date.now();
        for (const key of entries.keys()) {
          this._commitCooldown(cfg, state, key, now, cooldownOptions.get(key) || {});
        }
      }
    }
  }
  ...
  clearAgent(agentName) {
    ...
    for (const state of this._families.values()) {
      ...
      if (state.pendingCooldownOptions) {
        for (const key of [...state.pendingCooldownOptions.keys()]) {
          if (key === agentName || key.startsWith(prefix)) state.pendingCooldownOptions.delete(key);
        }
      }
    }
  }
  ...
  _commitCooldown(cfg, state, key, ts, options = {}) {
    state.lastEmitAt.set(key, ts);
    if (cfg.persistedCooldown && !options.skipPersistedWrite) cfg.persistedCooldown.write(key, ts);
  }
