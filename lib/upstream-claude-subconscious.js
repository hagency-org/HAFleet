...
import { memoryExportPolicyFromEnv } from './memory-export-policy.js';

const __filename = fileURLToPath(import.meta.url);
const REPO_ROOT = path.resolve(path.dirname(__filename), '..');
const HAFLEET_REPO_ROOT = String(process.env.HAFLEET_REPO_ROOT || REPO_ROOT).trim() || REPO_ROOT;
const DEFAULT_UPSTREAM_CLAUDE_SUBCONSCIOUS_ROOT = path.resolve(HAFLEET_REPO_ROOT, '..', 'claude-subconscious');

export const UPSTREAM_CLAUDE_SUBCONSCIOUS_ROOT = (
  process.env.UPSTREAM_CLAUDE_SUBCONSCIOUS_ROOT
  || DEFAULT_UPSTREAM_CLAUDE_SUBCONSCIOUS_ROOT
).trim() || DEFAULT_UPSTREAM_CLAUDE_SUBCONSCIOUS_ROOT;
...
/*
 * THE DEFAULT IS NOWHERE, NOT A THIRD PARTY.
 *
 * This returned `https://api.letta.com/v1` when nothing was configured, so a subsystem that
 * sends FULL session transcripts had a SaaS endpoint as its fallback. PRD R20 requires the
 * default to be off or an approved local destination, and the repo's own design note said
 * "that is a default to change before a customer asks"
 * (`docs/design/hafleet-as-pdu.md:239`).
 *
 * Nothing left a clean install before this, but only because the upstream checkout and the
 * API key were both absent — two accidents, not a policy. Returning null makes the absence
 * of a configured destination a decision the send path has to handle, which
 * `exportDestinationBlocker()` below does.
 */
...
/**
 * Why transcripts may not be sent, or null when they may.
 *
 * Checked alongside the existing `missing LETTA_API_KEY` blocker so a refusal reaches the
 * dashboard through machinery that already surfaces one, rather than failing silently.
 */
function exportDestinationBlocker(endpoint, env = process.env) {
  const verdict = memoryExportPolicyFromEnv(env).allows(endpoint);
...
async function fetchUpstreamAssistantMessages(apiKey, conversationUtils, conversationId, lastSeenMessageId) {
  ...
  const apiBase = normalizeApiBase(conversationUtils?.LETTA_API_BASE
    || (process.env.LETTA_BASE_URL ? `${process.env.LETTA_BASE_URL}/v1` : ''));
...
let upstreamEnvSerial = Promise.resolve();

async function runWithUpstreamEnv(envOverrides, fn) {
  const waitForTurn = upstreamEnvSerial.catch(() => {});
  let releaseTurn = () => {};
  upstreamEnvSerial = new Promise((resolve) => {
    releaseTurn = resolve;
  });
  await waitForTurn;
  const keys = [
...
export async function bootstrapUpstreamClaudeSubconsciousAgent({
  ...
  {
    // PRD R20: no approved destination means no export, stated rather than implied.
    const destinationBlocker = exportDestinationBlocker(process.env.LETTA_BASE_URL);
    if (destinationBlocker) {
      return { ok: false, blocked: true, blocker: destinationBlocker, paths, logs: [] };
    }
  }
  ...
  const result = await runWithUpstreamEnv({
    ...
    return {
      ...
      config,
      agent,
      lettaBaseUrl: (lettaBaseUrl || process.env.LETTA_BASE_URL || '').trim() || null,
    };
  });
