...
export function defaultAgentchatHomeDir(env = process.env) {
  // Explicit HAFLEET_HOMEDIR always wins.
  const explicit = normalizeAbsolutePath(env?.HAFLEET_HOMEDIR);
  if (explicit) return explicit;
  // v1.1: when HAFLEET_RUNTIME_DIR is set, place homes under it for dev/live isolation.
  const runtimeDir = normalizeAbsolutePath(env?.HAFLEET_RUNTIME_DIR);
  if (runtimeDir) return path.join(runtimeDir, 'homes');
  return path.resolve(os.homedir(), '.hafleet');
}
...
export function allAgentHomeRoots(env = process.env) {
  ...
  const legacy = path.resolve(os.homedir(), '.hafleet');
  const roots = [primary];
