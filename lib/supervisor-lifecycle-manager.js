...
function startTmuxSession(sessionName, workspaceDir, command) {
  ...
  // Auto-accept the --dangerously-skip-permissions bypass confirmation dialog.
  // The dialog defaults to "No, exit" — press Down to select "Yes" then Enter.
  // Send keys twice with delay to handle slow render.
  const autoAccept = (delayMs) => {
    setTimeout(() => {
      try {
        execFileSync('tmux', ['send-keys', '-t', `${sessionName}:0.0`, 'Down'], {
          timeout: 3000, stdio: ['ignore', 'ignore', 'pipe'],
        });
        setTimeout(() => {
          try {
  ...
  // Send initial bootstrap prompt after Claude Code is ready.
  // The prompt tells the supervisor to read its AGENTS.md and begin its assessment cycle.
  const sendInitialPrompt = (delayMs) => {
    setTimeout(() => {
      try {
        if (!tmuxSessionExists(sessionName)) return;
        const prompt = 'Read your AGENTS.md and begin your assessment cycle now.';
        execFileSync('tmux', ['send-keys', '-t', `${sessionName}:0.0`, prompt, 'Enter'], {
          timeout: 3000, stdio: ['ignore', 'ignore', 'pipe'],
        });
      } catch { /* session may not be ready yet */ }
    }, delayMs);
  };
  sendInitialPrompt(20000);
  sendInitialPrompt(30000);
}
...
function buildLaunchCommand(targetAgent, workspaceDir, selection, runtimeProfile) {
  const envPrefix = [
    ...
    `AGENT_CHAT_API=${shellQuote(process.env.AGENT_CHAT_API || '')}`,
    `AGENT_CHAT_BACKEND_PORT=${shellQuote(process.env.AGENT_CHAT_BACKEND_PORT || '')}`,
    `AGENTCHAT_SUPERVISED_AGENT_NAME=${shellQuote(targetAgent)}`,
...
export function createSupervisorLifecycleManager({
  ...
  function reconcile(targetAgent, now = Date.now()) {
    ...
    const command = buildLaunchCommand(targetAgent, workdir, selection, runtimeProfile);
    if (!command) {
...
export { tmuxSessionExists, killTmuxSession, startTmuxSession, tmuxPanePath, listTmuxSessions };
