...
function buildLaunchCommand(targetAgent, workspaceDir, selection, runtimeProfile) {
  ...
  const permissionMetadata = selection.framework === 'claude'
    ? [`AGENTCHAT_AGENT_PERMISSION_MODE=${shellQuote('auto')}`]
    : (selection.framework === 'codex' ? [
      `AGENTCHAT_AGENT_PERMISSION_LEVEL=${shellQuote('2')}`,
      `AGENTCHAT_AGENT_SANDBOX_MODE=${shellQuote('workspace-write')}`,
      `AGENTCHAT_AGENT_APPROVAL_POLICY=${shellQuote('on-request')}`,
    ] : []);
  const envPrefix = [
    `PATH=${shellQuote(process.env.PATH || '')}`,
    `AGENTCHAT_AGENT_SANDBOX=${shellQuote('1')}`,
    ...permissionMetadata,
