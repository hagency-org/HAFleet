 ...
 *
 * `bin/hafleet-up` sets `DATA_DIR="$RUNTIME_DIR/data/agents"` and the backend's storage adapter
 ...
 * the pane". `bin/hafleet-up` does this — `tmux send-keys "exec <script>"`, asserted by an exact
...
function buildLaunchCommand(targetAgent, workspaceDir, selection, runtimeProfile) {
   ...
   *
   * Both hafleet-up launchers do this — pinned by `launchers_clear_ambient_anthropic_key_without_
...
export function createSupervisorLifecycleManager({
  ...
  function reconcile(targetAgent, now = Date.now()) {
    ...
    try {
       ...
       * forbids by name.
       *
       * Verify-only and shared rather than reimplemented: `bin/hafleet-up` ESTABLISHES readiness and
