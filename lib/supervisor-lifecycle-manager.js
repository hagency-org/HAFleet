...
// --- tmux helpers (standalone, same patterns as supervisor/index.js) ---
...
// --- framework helpers (same as supervisor/index.js) ---
...
function buildLaunchCommand(supervisorName, workspaceDir, selection, runtimeProfile) {
  const envPrefix = [
    `PATH=${shellQuote(process.env.PATH || '')}`,
    `AGENTCHAT_SUPERVISED_AGENT_NAME=${shellQuote(supervisorName)}`,
...
export function createSupervisorLifecycleManager({
  ...
  function reconcile(targetAgent, now = Date.now()) {
    ...
    const command = buildLaunchCommand(supervisorName, workdir, selection, runtimeProfile);
    if (!command) {
  ...
  return { reconcile, sweepAll };
}

// Re-export tmux helpers for testing
export { tmuxSessionExists, killTmuxSession, startTmuxSession, tmuxPanePath };
