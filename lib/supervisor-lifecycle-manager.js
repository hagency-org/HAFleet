...
function startTmuxSession(sessionName, workspaceDir, command) {
  ...
  // Delayed slightly to allow the prompt to render.
  setTimeout(() => {
    try {
      execFileSync('tmux', ['send-keys', '-t', `${sessionName}:0.0`, 'Down', 'Enter'], {
        timeout: 3000, stdio: ['ignore', 'ignore', 'pipe'],
      });
    } catch { /* session may have already exited or prompt may have auto-accepted */ }
  }, 3000);
}
