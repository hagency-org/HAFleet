 ...
 *
 *   HAFLEET_SESSION_ALLOWLIST   only these sessions are managed
 *   HAFLEET_SESSION_DENYLIST    these sessions are never managed
 *
 ...
 *
 * @param {object} config
 * @param {string|undefined} config.allowlist raw HAFLEET_SESSION_ALLOWLIST
 * @param {string|undefined} config.denylist  raw HAFLEET_SESSION_DENYLIST
 */
export function createSessionPolicy({ allowlist, denylist } = {}) {
  ...
  const warnings = [];
  if (allowBroken) warnings.push('HAFLEET_SESSION_ALLOWLIST is set but lists no session; refusing all sessions');
  if (denyBroken) warnings.push('HAFLEET_SESSION_DENYLIST is set but lists no session; refusing all sessions');
...
export function sessionPolicyFromEnv(env = process.env) {
  return createSessionPolicy({
    allowlist: env.HAFLEET_SESSION_ALLOWLIST,
    denylist: env.HAFLEET_SESSION_DENYLIST,
  });
}
