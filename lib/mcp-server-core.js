...
import { z } from 'zod';
import { allAgentHomeRoots, v1AgentIdFromName } from './agent-home-v1.js';

function run(cmd) {
  return execSync(cmd, { encoding: 'utf-8', timeout: 3000 }).trim();
}

// Keys worth extracting from an ancestor's /proc/PID/environ when our env
// has been stripped (Codex sandbox).  Only fills missing process.env entries.
const ANCESTOR_ENV_KEYS = new Set([
  'AGENT_CHAT_RUNTIME_DIR', 'AGENTCHAT_HOMEDIR', 'AGENTCHAT_AGENT_STATE_DIR',
  'AGENT_CHAT_API', 'AGENT_CHAT_BACKEND_PORT', 'AGENT_CHAT_SERVER',
  'AGENT_CHAT_MCP_SERVER_NAME',
]);

function extractAncestorEnv(pid) {
  try {
    ...
    for (const entry of raw.toString('utf-8').split('\0')) {
      ...
      if (ANCESTOR_ENV_KEYS.has(key) && !process.env[key]) {
...
function detectAgentName() {
  ...
  // Fallback for sandboxed MCP subprocesses (e.g. Codex) that have no env
  // vars AND no TTY: walk /proc parent chain to find an ancestor with a real
  // TTY, then map that TTY to a tmux session.
  try {
    let pid = process.pid;
    ...
    while (pid > 1 && !visited.has(pid)) {
      ...
      // COMM can contain parens/spaces — find last ')' to parse safely.
      const cp = stat.lastIndexOf(')');
      if (cp < 0) break;
      const fields = stat.slice(cp + 2).split(' ');
      // [0]=state [1]=ppid [2]=pgrp [3]=session [4]=tty_nr
      const ppid = parseInt(fields[1], 10);
      const ttyNr = parseInt(fields[4], 10);
      if (ttyNr > 0) {
        // Decode tty_nr → /dev/pts/N. pts major range: 136-143.
        const major = (ttyNr >> 8) & 0xff;
        const minor = (ttyNr & 0xff) | ((ttyNr >> 12) << 8);
        if (major >= 136 && major <= 143) {
          const ptsPath = `/dev/pts/${(major - 136) * 256 + minor}`;
          if (!paneLines) {
            try { paneLines = run('tmux list-panes -a -F "#{pane_tty} #{session_name}"'); } catch { break; }
          }
          for (const line of paneLines.split('\n')) {
            const sp = line.indexOf(' ');
            if (sp < 0) continue;
            if (line.slice(0, sp).trim() === ptsPath) {
              const session = line.slice(sp + 1).trim();
              if (session) {
                // Extract env vars from this ancestor (or its parent) before
                // returning — fixes chicken-and-egg where loadMcpEnv needs
                // AGENT_CHAT_RUNTIME_DIR which Codex stripped.
                extractAncestorEnv(pid);
                return session;
              }
            }
          }
  ...
  // Simpler TTY fallback: this process's own TTY → tmux pane lookup.
  try {
...
// If critical env vars are missing (Codex sandbox strips them), try loading
// from per-agent mcp-env.json in the agent's state dir (written by agent-up).
// Scoped to the agent's own state dir to avoid cross-runtime bleed (dev vs live).
function loadMcpEnv(agentName) {
  function applyEnvFile(envFile) {
    try {
      ...
      process.stderr.write(`[mcp] Loaded env from ${envFile}\n`);
      return true;
    } catch { return false; }
  }

  // Fast path: AGENTCHAT_AGENT_STATE_DIR may already be set (from ancestor
  // /proc environ extraction or from .mcp.json env).
  const directStateDir = (process.env.AGENTCHAT_AGENT_STATE_DIR || '').trim();
  if (directStateDir && applyEnvFile(path.join(directStateDir, 'mcp-env.json'))) return;

  // Fallback: search all known home roots (needs AGENT_CHAT_RUNTIME_DIR,
  // which may have been set by extractAncestorEnv during proc-tree-walk).
  const agentId = v1AgentIdFromName(agentName);
  if (!agentId) return;
  for (const homeRoot of allAgentHomeRoots()) {
    if (applyEnvFile(path.join(homeRoot, 'agents', agentId, 'state', 'mcp-env.json'))) return;
  }
}
...
// Fill missing env vars from mcp-env file (supports Codex sandboxed env).
loadMcpEnv(AGENT_NAME);
