...
export const BENCHMARK_RUNTIME_SCHEMA = 'hafleet.benchmark.runtime-root/v1';
export const BENCHMARK_PROFILE_SCHEMA = 'hafleet.benchmark.profile-version/v1';
export const BENCHMARK_RUN_SCHEMA = 'hafleet.benchmark.run/v1';
export const BENCHMARK_TRIAL_SCHEMA = 'hafleet.benchmark.trial/v1';
...
export function defaultBenchmarkRuntimeRoot(env = process.env) {
  const explicit = normalizeText(env?.HAFLEET_BENCH_RUNTIME_DIR, 4096);
  if (explicit) return path.resolve(explicit);
  return path.resolve(os.homedir(), '.hafleet', 'bench-runtime');
}
...
export function ensureBenchmarkRuntimeRoot(rootPath, env = process.env) {
  ...
  if (!existsSync(layout.markerPath)) {
    writeJson(layout.markerPath, {
      ...
      notes: 'Benchmark runtime root isolated from HAFLEET_RUNTIME_DIR and live/human-operated agents.',
    });
  }
  return layout;
}
...
export function buildBenchmarkTrialPaths(rootPath, runId, trialId, trialAgentName, trialAgentId) {
  ...
  const v1Paths = buildV1AgentPaths(trialAgentName, { ...process.env, HAFLEET_HOMEDIR: runPaths.homesRoot }, trialAgentId);
  return {
    ...runPaths,
    trialRoot,
