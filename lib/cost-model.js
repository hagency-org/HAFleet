 ...
 *
 * DELIBERATELY UNWIRED — READ THIS BEFORE CONNECTING IT.
 *
 * Nothing imports this but its own test, and that is the decision rather than an
 * oversight. ADR-013's amendment of 2026-08-10 withdrew R8's `PriceBook` and
 * `BillingSource`: HAFleet's unit of account is the TOKEN. Converting tokens to money
 * depends on contract, plan, region and negotiated rate, none of which HAFleet observes,
 * so it is a separate problem for a separate system.
 *
 * It was written against §8's earlier claim that PriceBook was "retained and load-bearing",
 * before that claim was recognised as inconsistent with every interface in the product —
 * all of which are denominated in tokens, with `currency` appearing nowhere in the stores.
 *
 * Kept for one verified finding worth not re-deriving: pricing cache reads at the
 * fresh-input rate overstated a real session by 7.79× ($0.134775 became $1.049349 on
 * 339,166 cache reads against 2 fresh input tokens). If pricing is ever taken up, that is
 * the trap, and the subscription branch here records the other one — on a fixed plan the
 * marginal cost of a token is zero, so a per-engagement figure is an allocation of a bill
 * paid regardless, not a charge.
 *
