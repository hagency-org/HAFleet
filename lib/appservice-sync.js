 ...
 * a loop that holds the cursor hostage and fills the log with the same error. EIGHT attempts
 * spans ~4 minutes of backoff climb (1+2+4+8+16+32+60+60), far beyond any transient; after it,
 * the collector CIRCUIT-BREAKS the side and the failure rides postWarning to the operator.
 * The cursor never moves past an undelivered batch, so stopping is safe: nothing is lost,
 * nothing is skipped, and a restart resumes here. The value 8 is contractual — it is the
 * number the spec (specs/task-appservice-sync-intake.spec.md) states and a test pins.
 */
          ...
          if (handled?.status === 200) {
            ...
            if (attempts >= MAX_DELIVERY_ATTEMPTS) {
              ...
              if (!stats.circuitBreakWarned) {
                ...
                try { onCircuitBreak?.(side, { attempts, cursor: result.nextBatch ?? null, lastError: stats.lastError }); } catch { /* the hook must not break the break */ }
              }
              stopped = true;
              break;
            }
