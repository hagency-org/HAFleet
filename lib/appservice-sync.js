  ...
  const loop = (async () => {
    while (!stopped && shouldContinue()) {
      try {
        ...
        backoffMs = 1_000;
        /*
         * SUCCESS RESETS THE 401 BREAKER (5-r5): this sync poll answered, so the token WORKS
         * and the stretch of health begins anew — a future 401 may claim one fast re-login.
         * This is the ONLY reset of loginGeneration; the guard in the catch reads it, and the
         * reset's truth table is written there.
         */
        ...
        if (error?.status === 401) {
           ...
           *   NOTHING. It is reset to 0 ONLY by a successful sync poll (below, the `backoffMs
           *   = 1_000` line's neighbor) — success means the token WORKED, so we are healthy
           *   again and a future 401 may claim its one fast re-login.
           *
