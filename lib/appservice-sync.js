 ...
 * credential the router will not take) would otherwise be retried forever at the backoff cap —
 * a loop that holds the cursor hostage and fills the log with the same error. Twelve attempts
 * spans ~68s of backoff climb (1+2+4+…), far beyond any transient; after it, the loop STOPS and
 * the failure is left in stats for the operator. The cursor never moves past an undelivered
 * batch, so stopping is safe: nothing is lost, nothing is skipped, and a restart resumes here.
 */
        ...
        if (deliverable.length) {
          ...
          if (handled?.status === 200) {
            ...
            if (attempts >= MAX_DELIVERY_ATTEMPTS) {
              /*
               * GIVE UP LOUDLY. The cursor stays put, so a restart retries this batch; looping
               * here forever is the failure mode the cap exists to prevent (the runaway-loop OOM
               * of 01:59–02:02 was this shape: infinite retry, no exit, one fork's heap gone).
               */
              stats.gaveUp = true;
              logger.error?.(`[appservice-sync] router refused batch for side ${side} ${attempts} times — stopping the collector; `
                + `the cursor is held at ${result.nextBatch ?? '(none)'} and a restart will resume from it`);
              stopped = true;
              break;
            }
