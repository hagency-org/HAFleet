  ...
  const loop = (async () => {
    while (!stopped && shouldContinue()) {
      try {
          ...
          if (handled?.status === 200) {
            ...
            if (attempts >= MAX_DELIVERY_ATTEMPTS) {
              ...
              if (!stats.circuitBreakWarned) {
                ...
                try { onCircuitBreak?.(side, { attempts, cursor: result.nextBatch ?? null, heldCursor: since ?? null, failedNextBatch: result.nextBatch ?? null, lastError: stats.lastError }); } catch { /* the hook must not break the break */ }
              }
              stopped = true;
              break;
            }
        ...
        /*
         * TWO BACKOFF SHAPES (5-r3): a refusing BATCH is a delivery problem the operator must
         * see fast, so it retries at a FIXED 1s and circuit-breaks in ~8s. Everything else
         * (network, login) is a transient the loop can outwait, so it keeps the exponential
         * climb. batchAttempts > 0 is exactly the "we are inside a poison-batch retry" signal;
         * a successful delivery resets it to 0 in the 200 branch above.
         */
        const waitMs = (stats.batchAttempts ?? 0) > 0 ? POISON_RETRY_MS : backoffMs;
        await sleep(waitMs);
        if ((stats.batchAttempts ?? 0) === 0) {
