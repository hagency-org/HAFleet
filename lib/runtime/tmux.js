  ...
  return Object.freeze({
    ...
    async listPanes() {
      ...
      try {
        ...
        // An idle host with no server is reported ok-with-no-panes, not failed,
        // so callers do not treat "nothing running" as a fault.
        if (isTmuxEmptyServerError(error)) return emptyPaneListing(null);
        return emptyPaneListing(error);
      }

      const raw = String(stdout || '').trim();
      if (!raw) return emptyPaneListing(null);
