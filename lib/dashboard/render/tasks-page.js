...
(function(){
  ...
  function isTaskActionInFlight(id,action){return taskActionInFlight.has(taskActionKey(id,action))}
  function setTaskActionInFlight(id,action,inFlight){const key=taskActionKey(id,action);if(inFlight)taskActionInFlight.add(key);else taskActionInFlight.delete(key)}
  function currentTaskCommentDraft(id){
    const input=document.getElementById('task-comment-input');
    return input&&detailId===id?input.value:'';
  }
  ...
  function taskFilterKey(filters){
    return [filters.assignee||'',filters.status||''].join('\\u001f');
  }

  async function refresh(){
    if(refreshInFlight){refreshQueued=true;return}
    refreshInFlight=true;
    const requestFilters=getFilters();
    const requestFilterKey=taskFilterKey(requestFilters);
    try{
      const r=await fetch(taskListUrl(requestFilters));
      if(!r.ok)return;
      const nextTasks=normalizeTaskPayload(await r.json());
      if(requestFilterKey!==taskFilterKey(getFilters())){refreshQueued=true;return}
      taskCache=nextTasks;
  ...
  function renderDetail(task){
    ...
    let html='<span class="task-detail-back" onclick="backToList()">&#8592; Back to list</span>'
      ...
      +'<div class="task-detail-meta"><strong>Status:</strong> <select class="task-status-select" id="task-detail-status" onchange="changeStatus(\\''+esc(task.id)+'\\')"'+statusDisabled+'><option></option>';
    for(const s of statusOpts){html+='<option value="'+s+'"'+(selectedStatus===s?' selected':'')+'>'+s+'</option>';}
    html+='</select></div>';
    ...
    html+='<div class="task-comment-form"><textarea id="task-comment-input" placeholder="Add a comment..."'+commentDisabled+'></textarea><button id="task-comment-post" onclick="addComment(\\''+esc(task.id)+'\\')"'+commentDisabled+'>Post</button></div></div>';
    html+='<button class="task-delete-btn" id="task-delete-btn" onclick="deleteTask(\\''+esc(task.id)+'\\')"'+deleteDisabled+'>Delete Task</button>';
    root.innerHTML=html;
  ...
  window.addComment=async function(id){
    ...
    try{
      ...
      if(detailId===id&&liveInput)liveInput.value='';
      setTaskActionInFlight(id,action,false);
      await refresh();
