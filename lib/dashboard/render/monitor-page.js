...
(() => {
  ...
  function connectSSE() {
    connectDashboardStream((evtSource) => {
      ...
      evtSource.addEventListener('queue', (e) => {
        try { queueItems = normalizeArrayPayload(JSON.parse(e.data)); requestQueueRender(false); } catch (err) {
          console.debug('[sse] queue parse skipped:', err.message);
        }
      });
      evtSource.addEventListener('reminders', (e) => {
        try { reminderItems = normalizeArrayPayload(JSON.parse(e.data)); renderReminderPanel(); } catch (err) {
  ...
  async function init() {
    ...
    // Initial state
    try { const r = await fetch('/api/queue'); queueItems = normalizeArrayPayload(await r.json()); requestQueueRender(true); } catch (e) {
      console.debug('[init] queue load skipped:', e.message);
    }
    try { const r = await fetch('/api/reminders'); reminderItems = normalizeArrayPayload(await r.json()); renderReminderPanel(); } catch (e) {
