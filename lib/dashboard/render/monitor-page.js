...
(() => {
  ...
  function applyQueuePayload(payload, force = false) {
    queueItems = normalizeQueuePayload(payload).filter(item => !pendingQueueActionIds.has(String(item.id)));
  ...
  function applyReminderPayload(payload, force = false) {
    reminderItems = normalizeReminderPayload(payload).filter(item => !pendingReminderCancelIds.has(String(item.id)));
