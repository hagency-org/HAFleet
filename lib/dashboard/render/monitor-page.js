...
.monitor-brand{color:rgba(0,240,255,0.5);letter-spacing:2px;white-space:nowrap}
.monitor-nav{display:flex;align-items:center;gap:14px;min-width:0;overflow-x:auto;scrollbar-width:none}
.monitor-nav::-webkit-scrollbar{display:none}
.monitor-nav a{color:rgba(0,240,255,0.38);text-decoration:none;letter-spacing:1px;white-space:nowrap}
.monitor-nav a:hover{color:rgba(0,240,255,0.8)}
.monitor-nav a.active{color:#00f0ff;text-shadow:0 0 9px rgba(0,240,255,0.35)}
.monitor-nav a.active::after{content:'';display:block;height:1px;margin-top:3px;background:#00f0ff;box-shadow:0 0 6px rgba(0,240,255,0.55)}
.monitor-alert-badge{margin-left:3px;color:#ff7373;letter-spacing:0;text-shadow:none}
.monitor-new-agent{margin-left:auto;font-size:10px;color:rgba(0,240,255,0.4);background:none;border:1px solid rgba(0,240,255,0.15);padding:2px 8px;cursor:pointer;letter-spacing:1px;font-family:inherit;white-space:nowrap}
#agent-buttons-wrap{
...
/* Two lines: identity, then state. The transition list was narrowed from "all"
   above, because "all" animated the height change on every re-render and made the
   list shimmer. Backticks are avoided in these comments: this stylesheet lives in
   a JS template literal, and one would end it. */
.agent-btn-line{display:inline-flex;align-items:center;gap:6px}
.agent-btn-name{font-size:11px}
.agent-btn-state{
  font-size:9px;letter-spacing:0.04em;opacity:0.75;
  font-variant-numeric:tabular-nums;
}
/* Durations tick every second, so the digits must not reflow the button. */
.agent-btn.active-agent .agent-btn-state{color:#34d399}
.agent-btn.inactive-agent .agent-btn-state{color:rgba(255,255,255,0.45)}
.agent-btn.remote-agent .agent-btn-state{color:#a78bfa}
#action-notice{
  position:fixed;bottom:14px;left:50%;transform:translateX(-50%) translateY(8px);
  padding:6px 14px;border-radius:6px;font-size:11px;letter-spacing:0.02em;
  pointer-events:none;opacity:0;transition:opacity .18s,transform .18s;
  z-index:50;max-width:70vw;
}
#action-notice.visible{opacity:1;transform:translateX(-50%) translateY(0)}
.action-notice-ok{background:rgba(52,211,153,0.14);border:1px solid #34d399;color:#34d399}
.action-notice-fail{background:rgba(248,113,113,0.16);border:1px solid #f87171;color:#f87171}
.agent-btn-tag{
  font-size:8px;letter-spacing:0.08em;opacity:0.65;
...
<div id="app">
  ...
  <div id="main-row">
    ...
    <div id="monitor-panel">
      <div class="monitor-header" style="display:flex;align-items:center;gap:12px">
        <span class="monitor-brand">AGENT MONITOR</span>
        <nav class="monitor-nav" aria-label="Dashboard">
          <a class="active" aria-current="page" href="/">MONITOR</a>
          <a href="/projects">PROJECTS</a>
          <a href="/tasks">TASKS</a>
          <a href="/pool">POOL</a>
          <a href="/alerts">ALERTS <span id="alert-badge" class="monitor-alert-badge" style="display:none"></span></a>
          <a href="/config">CONFIG</a>
          <a href="/project-sides">SIDES</a>
        </nav>
        <button class="monitor-new-agent" id="btn-new-agent" onclick="openNewAgentModal()">+ NEW</button>
      </div>
      ...
      <div class="monitor-bar">
        ...
        <span class="monitor-bar-btns">
          ...
          <button id="btn-speed" style="display:none" title="How often this browser re-reads the pane. Does not affect the agent.">Refresh: 10/sec</button>
          <button id="btn-audit" style="display:none" onclick="openAuditPage()" title="Open this agent's detail page">Agent details</button>
          <button id="btn-pause" style="display:none" title="Stop refreshing this view. The agent keeps running.">&#9646;&#9646; Pause display</button>
        </span>
      </div>
...
var _naPresets = [];
function normalizeNewAgentPresetPayload(payload) {
  ...
  return payload.map(function(item) {
    ...
    return Object.assign({}, item, {
      id: id,
      name: name,
      framework: typeof item.framework === 'string' && item.framework.trim() ? item.framework.trim() : '?',
    });
  }).filter(Boolean);
}
window.openNewAgentModal = async function() {
  ...
  try {
    ...
    if (r.ok) _naPresets = normalizeNewAgentPresetPayload(await r.json());
  } catch {}
  sel.innerHTML = '';
...
(() => {
  ...
  function normalizeArrayPayload(payload) {
    return Array.isArray(payload) ? payload.filter(function(item) { return item && typeof item === 'object'; }) : [];
  }
  function normalizePresetPayload(payload) {
    return normalizeArrayPayload(payload).map(function(item) {
      var id = typeof item.id === 'string' ? item.id.trim() : '';
      var name = typeof item.name === 'string' ? item.name.trim() : '';
      if (!id || !name) return null;
      return Object.assign({}, item, {
  ...
  function normalizeQueuePayload(payload) {
    return normalizeArrayPayload(payload).map(function(item) {
      ...
      var queuedAt = Number(item.queuedAt);
      return Object.assign({}, item, {
        id: id,
        ...
        targetObservation: item.targetObservation && typeof item.targetObservation === 'object' ? item.targetObservation : {},
      });
    }).filter(Boolean);
  }
  function normalizeReminderPayload(payload) {
    return normalizeArrayPayload(payload).map(function(item) {
      var id = Number.parseInt(item.id, 10);
      if (!Number.isFinite(id)) return null;
      var remainingMs = Number(item.remainingMs);
      return Object.assign({}, item, {
        id: id,
  ...
  function normalizeMessageLogPayload(payload) {
    return normalizeArrayPayload(payload).map(normalizeMessageLogRow).filter(Boolean);
  }
  function normalizeAgentStatusPayload(payload) {
    const now = Date.now();
    return normalizeArrayPayload(payload).map(row => {
      const name = typeof row.name === 'string' ? row.name.trim() : '';
      if (!name) return null;
      return {
        ...row,
        name,
        tmux: typeof row.tmux === 'string' ? row.tmux : '',
        environment: typeof row.environment === 'string' && row.environment.trim() ? row.environment.trim() : 'live',
        ...
        lastTmuxActivitySec: toNonNegInt(row.lastTmuxActivitySec, 0),
        _localTickAt: now,
      };
    }).filter(Boolean);
  }
  function normalizeAlertStats(payload) {
    const src = payload && typeof payload === 'object' ? payload : {};
    return {
      total: toNonNegInt(src.total, 0),
      byStatus: src.byStatus && typeof src.byStatus === 'object' ? src.byStatus : {},
      bySeverity: src.bySeverity && typeof src.bySeverity === 'object' ? src.bySeverity : {},
    };
  }
  ...
  function addLogEntry(msg) {
    ...
    const row = normalizeMessageLogRow(msg);
    if (!row) return;
    const ts = new Date(row.ts).toLocaleTimeString();
    const payload = (row.payload || '').slice(0, 120);
    const div = document.createElement('div');
    div.className = 'log-entry';
    div.innerHTML =
      '<span class="ts">' + ts + '</span> '
      + '<span class="from">' + esc(row.from) + '</span> '
      + '<span class="arrow">&#10145;</span> '
      + '<span class="to">' + esc(row.to) + '</span> '
  ...
  const pendingQueueActionIds = new Set();
  const tombstonedQueueActionIds = new Set();
  let queueRenderLocked = false;
  let queueRenderPending = false;
  let lastQueueHtml = '';
  function queueTombstoneKey(item) {
    if (!item || typeof item !== 'object') return '';
    const meta = item.notifyMeta && typeof item.notifyMeta === 'object' ? item.notifyMeta : {};
    const messageIds = Array.isArray(meta.messageIds) ? meta.messageIds.join(',') : '';
    return [
      item.id,
      item.from,
      item.to,
  ...
  function queueRenderedTombstoneKey(id) {
    const targetId = String(id);
    for (const row of queueList.querySelectorAll('.queue-item[data-id]')) {
      if (row.getAttribute('data-id') === targetId) return row.getAttribute('data-tombstone-key') || '';
    }
    return '';
  }
  ...
  function queueIdleInfo(item) {
    ...
    if (observationState === 'pane-missing' || (!observationState && idleMs < 0)) {
      ...
      text = 'idle ' + (s < 60 ? s + 's' : Math.floor(s/60) + 'm' + (s%60) + 's') + ' (delivering soon)';
      className = 'qi-idle-ready';
    } else {
      const s = Math.floor(idleMs / 1000);
      text = 'target active (idle ' + s + 's / ' + IDLE_THRESHOLD_SEC + 's)';
      className = 'qi-idle-busy';
    }
  ...
  function updateQueueTimersInPlace() {
    ...
    for (const row of queueList.querySelectorAll('.queue-item[data-id]')) {
      ...
      const idleEl = row.querySelector('.qi-idle');
      if (idleEl) {
        const info = queueIdleInfo(item);
        idleEl.textContent = info.text;
        idleEl.className = 'qi-idle ' + info.className;
      }
    }
  }

  function applyQueuePayload(payload, force = false) {
    queueItems = normalizeQueuePayload(payload).filter(item => {
      const id = String(item.id);
      return !pendingQueueActionIds.has(id) && !tombstonedQueueActionIds.has(queueTombstoneKey(item));
    });
    requestQueueRender(force);
  }

  function isSuccessfulDeleteResponse(res, body) {
    if (body && body.queue_remove_failed === true) return false;
  ...
  function isTerminalQueueActionResponse(res, body, action) {
    if (action !== 'send') return isSuccessfulDeleteResponse(res, body);
    if (body && body.requeued === true) return false;
    if (res.status === 404) return true;
    if (body && body.requeued === false) return true;
    if (!res.ok) return false;
    if (body && body.ok === false) return false;
    return true;
  }
  ...
  function renderQueuePanel() {
    ...
    const html = queueItems.map(item => {
      ...
      const idleInfo = queueIdleInfo(item);
      const redir = item.redirectedFrom ? ' <span class="qi-redir">(was ' + esc(item.redirectedFrom) + ')</span>' : '';
      return '<div class="queue-item" data-id="' + item.id + '" data-tombstone-key="' + esc(queueTombstoneKey(item)) + '">'
        ...
        + '<div class="qi-wait"></div>'
        + '<div class="qi-idle ' + idleInfo.className + '"></div>'
        + '<div class="qi-actions">'
        + '<button class="qi-btn qi-btn-send" title="Deliver immediately, without waiting for the agent to go idle." onclick="queueAction(' + item.id + ',\\'send\\')">Send now &middot; skip wait</button>'
        + '<button class="qi-btn qi-btn-cancel" title="Drop this queued message. It is not delivered." onclick="queueAction(' + item.id + ',\\'cancel\\')">Discard</button>'
        + '</div></div>';
    }).join('');
  ...
  /**
   * Say out loud what an action did.
   *
   * Queue actions removed their row optimistically and, on failure, put it back and
   * wrote to console.debug. Nothing on screen distinguished success from failure,
   * and a restored row read as a newly arrived one. This is the smallest thing that
   * makes the outcome visible, and it never covers the terminal.
   */
  let actionNoticeTimer = null;
  function announceAction(kind, text) {
    let el = document.getElementById('action-notice');
    if (!el) {
    ...
    el.textContent = text;
    el.className = 'action-notice-' + (kind === 'ok' ? 'ok' : 'fail') + ' visible';
    if (actionNoticeTimer) clearTimeout(actionNoticeTimer);
    // A failure stays up longer: it is the one that needs acting on.
    actionNoticeTimer = setTimeout(() => { el.classList.remove('visible'); },
      kind === 'ok' ? 2500 : 8000);
  }

  window.queueAction = async function(id, action) {
    const pendingId = String(id);
    if (pendingQueueActionIds.has(pendingId)) return;
    pendingQueueActionIds.add(pendingId);
    // Optimistic: remove immediately, restore on failure
    const removed = queueItems.find(i => String(i.id) === pendingId);
    const removedTombstoneKey = removed ? queueTombstoneKey(removed) : queueRenderedTombstoneKey(pendingId);
    queueItems = queueItems.filter(i => String(i.id) !== pendingId);
    ...
    try {
      ...
      if (action === 'send') {
        ...
        const targetAgent = (removed && typeof removed.to === 'string')
          ...
          : '';
        const isBackendNotification = removed && removed.from === 'hafleet-backend';
      ...
      if (!isTerminalQueueActionResponse(res, body, action)) {
        throw new Error((body && body.reason) || ('HTTP ' + res.status));
      }
      if (removedTombstoneKey) tombstonedQueueActionIds.add(removedTombstoneKey);
      ...
      announceAction('ok', (action === 'send' ? 'Sent' : 'Discarded') + ' queued message #' + pendingId);
    } catch (e) {
      // Previously console.debug only. The row is removed optimistically and put back
      // on failure, so a failed send looked identical to a new event arriving, and
      // 'sent', 'discarded' and 'still queued' were indistinguishable. The endpoint
      // really does refuse: 409 already-delivering, 503 queue-persist-failed.
      console.debug('[monitor] queue action failed, restoring removed entry:', e.message);
      announceAction('fail', (action === 'send' ? 'Send' : 'Discard') + ' failed for #' + pendingId + ': ' + e.message);
      if (removed && !queueItems.some(i => String(i.id) === pendingId)) queueItems.push(removed);
    } finally {
      pendingQueueActionIds.delete(pendingId);
      requestQueueRender(true);
    }
  };
  ...
  const pendingReminderCancelIds = new Set();
  const tombstonedReminderCancelIds = new Set();
  let lastReminderHtml = '';
  function reminderTombstoneKey(item) {
    if (!item || typeof item !== 'object') return '';
    return [
      item.id,
      item.target,
  ...
  function reminderRenderedTombstoneKey(id) {
    const targetId = String(id);
    for (const row of reminderList.querySelectorAll('.reminder-item[data-id]')) {
      if (row.getAttribute('data-id') === targetId) return row.getAttribute('data-tombstone-key') || '';
    }
    return '';
  }
  function updateReminderTimersInPlace() {
    const byId = new Map(reminderItems.map(item => [String(item.id), item]));
    for (const row of reminderList.querySelectorAll('.reminder-item[data-id]')) {
      const id = row.getAttribute('data-id');
      const item = byId.get(String(id));
      if (!item) continue;
      const countdownEl = row.querySelector('.ri-countdown');
      if (countdownEl) countdownEl.textContent = '\\u23f0 ' + fmtCountdown(item.remainingMs || 0);
    }
  }
  function applyReminderPayload(payload, force = false) {
    reminderItems = normalizeReminderPayload(payload).filter(item => {
      const id = String(item.id);
      return !pendingReminderCancelIds.has(id) && !tombstonedReminderCancelIds.has(reminderTombstoneKey(item));
    });
    renderReminderPanel(force);
  }
  function renderReminderPanel() {
    ...
    const html = reminderItems.map(item => {
      ...
      return '<div class="reminder-item" data-id="' + item.id + '" data-tombstone-key="' + esc(reminderTombstoneKey(item)) + '">'
        + '<div class="ri-target">' + esc(item.target) + '</div>'
        + '<div class="ri-msg">' + esc(msg) + '</div>'
        + '<div class="ri-countdown"></div>'
        + '<div class="ri-created">set at ' + created + '</div>'
        + '<div class="ri-actions">'
        + '<button class="ri-btn-cancel" title="Drop this reminder. It will not fire." onclick="cancelReminder(' + item.id + ')">Discard</button>'
        + '</div></div>';
    }).join('');
    if (html !== lastReminderHtml) { reminderList.innerHTML = html; lastReminderHtml = html; }
    updateReminderTimersInPlace();
  }
  setInterval(renderReminderPanel, 2000);

  window.cancelReminder = async function(id) {
    const pendingId = String(id);
    if (pendingReminderCancelIds.has(pendingId)) return;
    pendingReminderCancelIds.add(pendingId);
    // Optimistic: remove immediately, restore on failure
    const removed = reminderItems.find(i => String(i.id) === pendingId);
    const removedTombstoneKey = removed ? reminderTombstoneKey(removed) : reminderRenderedTombstoneKey(pendingId);
    reminderItems = reminderItems.filter(i => String(i.id) !== pendingId);
    renderReminderPanel();
    try {
      const res = await fetch('/api/reminders/' + id, { method: 'DELETE' });
      let body = null;
      try { body = await res.json(); } catch (e) {
        console.debug('[agent-detail] reminder cancel response parse skipped:', e.message);
      }
      if (!isSuccessfulDeleteResponse(res, body)) throw new Error((body && body.reason) || ('HTTP ' + res.status));
      if (removedTombstoneKey) tombstonedReminderCancelIds.add(removedTombstoneKey);
      announceAction('ok', 'Discarded reminder #' + pendingId);
    } catch (e) {
      // Same silent-failure shape as the queue path: removed optimistically, put
      // back on failure, console.debug only.
      console.debug('[monitor] reminder cancel failed, restoring removed entry:', e.message);
      announceAction('fail', 'Discard failed for reminder #' + pendingId + ': ' + e.message);
      if (removed && !reminderItems.some(i => String(i.id) === pendingId)) reminderItems.push(removed);
    } finally {
      pendingReminderCancelIds.delete(pendingId);
      renderReminderPanel();
    }
  };
  ...
  let agentStatusInFlight = false;
  let agentStatusRefreshQueued = false;
  ...
  function updateSpeedButton() {
    ...
    if (terminalTurboMode) {
      btnSpeed.textContent = 'Refresh: 10/sec';
      btnSpeed.classList.add('turbo');
    } else {
      btnSpeed.textContent = 'Refresh: Eco';
  ...
  function scheduleDetailRefresh() {
    ...
    detailRefreshTimer = setInterval(() => {
      refreshAgentDetailIfIdle();
    }, interval);
  }

  function refreshAgentDetailIfIdle() {
    if (!monitoredAgent || agentDetailAbortController) return false;
  ...
  btnPause.addEventListener('click', () => {
    ...
    if (monitorPaused) {
      btnPause.innerHTML = '&#9654; Resume display';
      btnPause.classList.add('paused');
    } else {
      btnPause.innerHTML = '&#9646;&#9646; Pause display';
  ...
  function selectAgent(agent) {
    ...
    monitorUrl.searchParams.set('agent', agent.name);
  ...
  async function fetchAgentDetail(name, options = {}) {
    ...
    try {
      ...
      if (detailRespRaw.status !== 'fulfilled') {
        showAgentDetailError(targetName, 'Summary unavailable: ' + (detailRespRaw.reason?.message || 'request failed'));
        return;
      }
      const res = detailRespRaw.value;
      if (!res.ok) {
        showAgentDetailError(targetName, 'Summary unavailable (HTTP ' + res.status + ').');
        return;
      }
      const d = await res.json().catch(() => null);
      if (!d || typeof d !== 'object') {
        showAgentDetailError(targetName, 'Summary unavailable: invalid response.');
        return;
      }
      if (requestSeq !== agentDetailRequestSeq) return;
      if (!monitoredAgent || monitoredAgent.name !== targetName) return;
  ...
  async function fetchTerminal() {
    const targetAgent = monitoredAgent;
    if (!targetAgent || monitorPaused) return;
    const targetName = String(targetAgent.name || '');
    if (terminalFetchInFlight.has(targetName)) return;
    const requestSeq = ++terminalFetchSeq;
    terminalFetchInFlight.add(targetName);
    const isCurrentRequest = () => monitoredAgent && monitoredAgent.name === targetName && requestSeq === terminalFetchSeq;
    try {
      let url = '/api/tmux/capture/' + encodeURIComponent(targetAgent.tmux);
      if (targetAgent.remote && targetAgent.server) {
        url += '?server=' + encodeURIComponent(targetAgent.server);
      }
      const headers = {};
      ...
      if (isCurrentRequest()) terminalEl.textContent = '[Error fetching terminal: ' + e.message + ']';
    } finally {
      terminalFetchInFlight.delete(targetName);
    }
  }
  ...
  function renderAgentButtons(agents) {
    ...
    // Each button carries its own state, so the first screen answers "what needs
    // attention?" without selecting anything. Before this it showed a glyph and a
    // name, and ACTIVE/IDLE plus duration existed only after you clicked an agent —
    // so finding trouble meant clicking every agent in turn.
    //
    // The status string is not new: runtimeStatusText() already produced exactly
    // this text for the selected-agent badge and the detail panel. It is only
    // rendered in one more place.
    //
    // REMOTE and SUP are spelled out. The glyph stays as a secondary cue, but
    // colour and shape alone required knowing a legend that is nowhere on the page.
    function agentBtnHtml(a) {
      ...
      const stateText = runtimeStatusText(
        isActive,
        toNonNegInt(a.activeDurationSec, 0),
        toNonNegInt(a.idleDurationSec, 0),
      );
      const tags = [];
      if (isRemote) tags.push('REMOTE' + (a.alive ? '' : ' DOWN'));
      if (isSupervisor) tags.push('SUP');
      const tagHtml = tags.length
        ? '<span class="agent-btn-tag">' + esc(tags.join(' \u00b7 ')) + '</span>'
        : '';
      // data-state-for lets the local duration ticker rewrite just this text,
      // instead of re-rendering the list. A re-render would re-sort it and move
      // buttons out from under the pointer mid-scan.
      return '<button class="' + cls + '" data-name="' + esc(a.name) + '" data-tmux="' + esc(a.tmux || '') + '"'
        + (targetName ? ' data-sup-target="' + esc(targetName) + '"' : '') + '>'
        + '<span class="agent-btn-line">'
        + '<span class="dot">' + dot + '</span>'
        + '<span class="agent-btn-name">' + esc(a.name) + '</span>'
        + tagHtml
        + '</span>'
        + '<span class="agent-btn-state" data-state-for="' + esc(a.name) + '">' + esc(stateText) + '</span>'
        + '</button>';
    }
    let html = '';
  ...
  async function fetchAgentStatus(_reason = 'poll') {
    if (agentStatusInFlight) { agentStatusRefreshQueued = true; return; }
    agentStatusInFlight = true;
    try {
      const res = await fetch('/api/agents/status');
      if (!res.ok) return;
      const payload = await res.json();
      const normalized = normalizeAgentStatusPayload(payload);
      ...
      agentStatusInFlight = false;
      if (agentStatusRefreshQueued) {
        agentStatusRefreshQueued = false;
        fetchAgentStatus(_reason || 'queued').catch(() => {});
      }
    }
  }
  ...
  /**
   * Rewrite each agent button's state text from the ticked snapshot.
   *
   * Deliberately NOT a re-render. renderAgentButtons() re-sorts by active/idle and
   * idle duration, so calling it once a second would shuffle buttons under the
   * pointer while an operator is reading them — the durations would be live and
   * the list unusable. Updating textContent in place leaves position, hover, and
   * focus untouched.
   */
  function updateAgentButtonStates() {
    if (!agentButtonsEl) return;
    const nodes = agentButtonsEl.querySelectorAll('[data-state-for]');
    if (!nodes.length) return;
    const byName = new Map(agentStatusList.map((a) => [a.name, a]));
    for (const node of nodes) {
      const snap = byName.get(node.getAttribute('data-state-for'));
      if (!snap) continue;
      const isActive = typeof snap.activeNow === 'boolean' ? snap.activeNow : !!snap.active;
      const text = runtimeStatusText(
        isActive,
        toNonNegInt(snap.activeDurationSec, 0),
        toNonNegInt(snap.idleDurationSec, 0),
      );
      if (node.textContent !== text) node.textContent = text;
      // The colour class lives on the button, so a state flip has to move with it.
      const btn = node.closest('.agent-btn');
      if (btn && !btn.classList.contains('remote-agent')) {
        btn.classList.toggle('active-agent', isActive);
        btn.classList.toggle('inactive-agent', !isActive);
      }
    }
  }
  ...
  // ── SSE ─────────────────────────────────────
  function connectSSE() {
    connectDashboardStream((evtSource) => {
      evtSource.onmessage = (e) => {
        try { addLogEntry(normalizeMessageLogRow(JSON.parse(e.data))); } catch (err) {
          console.debug('[sse] message parse skipped:', err.message);
        }
      };
      evtSource.addEventListener('queue', (e) => {
        try { applyQueuePayload(JSON.parse(e.data), false); } catch (err) {
          console.debug('[sse] queue parse skipped:', err.message);
        }
      });
      evtSource.addEventListener('reminders', (e) => {
        try { applyReminderPayload(JSON.parse(e.data), false); } catch (err) {
  ...
  async function init() {
    ...
    try {
      ...
      const msgs = normalizeMessageLogPayload(await res.json());
      for (const msg of msgs) addLogEntry(msg);
    } catch (e) { console.error('messages load failed:', e); }
    // Initial state
    try { const r = await fetch('/api/queue'); applyQueuePayload(await r.json(), true); } catch (e) {
      console.debug('[init] queue load skipped:', e.message);
    }
    try { const r = await fetch('/api/reminders'); applyReminderPayload(await r.json(), true); } catch (e) {
      console.debug('[init] reminders load skipped:', e.message);
    }
    await fetchAgentStatus('init');
    const requestedAgentName = new URLSearchParams(window.location.search).get('agent');
    if (requestedAgentName) {
      const requestedAgent = agentStatusList.find(agent => agent.name === requestedAgentName);
      if (requestedAgent) selectAgent(requestedAgent);
    }
    ...
    connectSSE();
    // Alert badge
    let alertBadgeInFlight=false;
    let alertBadgeQueued=false;
    async function refreshAlertBadge(){
      if(alertBadgeInFlight){alertBadgeQueued=true;return}
      alertBadgeInFlight=true;
      try{
        const r=await fetch('/api/alerts/stats');
        if(!r.ok)return;
        const s=normalizeAlertStats(await r.json());
        const open=toNonNegInt(s.byStatus.open,0)+toNonNegInt(s.byStatus.acknowledged,0)+toNonNegInt(s.byStatus.assigned,0);
        const crit=toNonNegInt(s.bySeverity.critical,0);
        ...
        alertBadgeInFlight=false;
        if(alertBadgeQueued){alertBadgeQueued=false;refreshAlertBadge()}
      }
    }
    refreshAlertBadge();
