...
(() => {
  ...
  function connectSSE() {
    connectDashboardStream((evtSource) => {
      ...
      evtSource.addEventListener('queue', (e) => {
        try { queueItems = normalizeQueuePayload(JSON.parse(e.data)); requestQueueRender(false); } catch (err) {
          console.debug('[sse] queue parse skipped:', err.message);
        }
      });
      evtSource.addEventListener('reminders', (e) => {
        try { reminderItems = normalizeReminderPayload(JSON.parse(e.data)); renderReminderPanel(); } catch (err) {
  ...
  async function init() {
    ...
    // Initial state
    try { const r = await fetch('/api/queue'); queueItems = normalizeQueuePayload(await r.json()); requestQueueRender(true); } catch (e) {
      console.debug('[init] queue load skipped:', e.message);
    }
    try { const r = await fetch('/api/reminders'); reminderItems = normalizeReminderPayload(await r.json()); renderReminderPanel(); } catch (e) {
