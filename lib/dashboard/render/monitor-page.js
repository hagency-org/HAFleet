...
(() => {
  ...
  function applyQueuePayload(payload, force = false) {
    queueItems = normalizeQueuePayload(payload).filter(item => {
      ...
      return !pendingQueueActionIds.has(id) && !tombstonedQueueActionIds.has(id);
    });
  ...
  window.queueAction = async function(id, action) {
    ...
    try {
      ...
      tombstonedQueueActionIds.add(pendingId);
  ...
  function applyReminderPayload(payload, force = false) {
    reminderItems = normalizeReminderPayload(payload).filter(item => {
      const id = String(item.id);
      return !pendingReminderCancelIds.has(id) && !tombstonedReminderCancelIds.has(id);
    });
  ...
  function renderReminderPanel() {
    ...
    const html = reminderItems.map(item => {
      ...
      return '<div class="reminder-item" data-id="' + item.id + '">'
  ...
  window.cancelReminder = async function(id) {
    ...
    try {
      ...
      tombstonedReminderCancelIds.add(pendingId);
    } catch (e) {
