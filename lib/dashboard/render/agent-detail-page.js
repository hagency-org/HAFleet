...
export function shouldPreserveDetailSettings({
  ...
  if (!forceDetailRender) return true;
  if (saveEditVersion === null || saveEditVersion === undefined) return true;
  return currentEditVersion !== saveEditVersion;
}
...
${shouldPreserveDetailSettings.toString()}
(() => {
  const agent = ${JSON.stringify(agentName)};
  const dmStorage = safeStorage('localStorage');
  const taskFilterStorage = safeStorage('sessionStorage');
  const AGENT_DETAIL_REFRESH_VISIBLE_MS = 5000;
  const AGENT_DETAIL_REFRESH_HIDDEN_MS = 30000;
  ...
  function normalizePresetPayload(payload) {
    return normalizeObjectArray(payload).map((item) => {
      ...
      return {
        ...
        extraArgs: typeof item.extraArgs === 'string' ? item.extraArgs : null,
  ...
  function normalizeAgentStatusPayload(payload) {
    return normalizeObjectArray(payload).map((item) => {
      ...
      return {
        ...item,
        name,
        activeNow: typeof item.activeNow === 'boolean' ? item.activeNow : !!item.active,
        activeDurationSec: toInt(item.activeDurationSec, 0),
        idleDurationSec: toInt(item.idleDurationSec, 0),
      };
    }).filter(Boolean);
  }
  function normalizeDmMessagesPayload(payload) {
    return normalizeObjectArray(payload?.messages).map((item) => ({
      ...item,
      from: typeof item.from === 'string' ? item.from : 'unknown',
      type: typeof item.type === 'string' ? item.type : 'inform',
      source: typeof item.source === 'string' ? item.source : '',
      summary: typeof item.summary === 'string' ? item.summary : '',
      full: typeof item.full === 'string' ? item.full : '',
      at: item.at || item.ts || item.time || null,
    }));
  }
  ...
  function markDetailEdited() {
    detailEditVersion += 1;
    syncDetailDirtyStatus();
  }

  function beginDetailSave() {
    detailSaveInFlight = true;
    return detailEditVersion;
  }

  function refreshAfterDetailSave(saveEditVersion) {
    return refresh({ forceDetailRender: true, saveEditVersion });
  }
  ...
  function bindDetailEditors() {
    ...
    ids.forEach((id) => {
      ...
      el.addEventListener(evt, markDetailEdited);
    });
  }
  ...
  function setActiveTab(nextTab, options = {}) {
    ...
    if (next === 'tasks') {
      // Auto-set assignee filter to the detail agent unless the user changed it.
      const filterEl = document.getElementById('task-filter-assignee');
      const u = new URL(window.location);
      const savedAssignee = storageGet(taskFilterStorage, 'task_filter_assignee');
      if (filterEl && agent && !filterEl._userOverride && !u.searchParams.has('assignee') && !savedAssignee) {
        filterEl.value = agent;
        storageSet(taskFilterStorage, 'task_filter_assignee', agent);
        u.searchParams.set('assignee', agent);
  ...
  function getDmOperatorName() {
    return sanitizeOperatorName(storageGet(dmStorage, DM_LS_KEY));
  }
  {
    ...
    if (nameInput) {
      ...
      nameInput.addEventListener('input', () => {
        ...
        nameInput.value = v;
        storageSet(dmStorage, DM_LS_KEY, v);
      });
    }
  }
  let dmLoaded = false;
  ...
  let dmHistoryInFlight = false;
  let dmHistoryQueued = false;
  ...
  async function loadDmHistory() {
    if (dmHistoryInFlight) { dmHistoryQueued = true; return false; }
    dmHistoryInFlight = true;
    try {
      ...
      dmMessages = normalizeDmMessagesPayload(data);
      dmLoaded = true;
      renderDmMessages();
    } catch (e) {
      console.warn('[dm] load error:', e);
    } finally {
      dmHistoryInFlight = false;
      if (dmHistoryQueued) {
        dmHistoryQueued = false;
        loadDmHistory();
      }
    }
  }
  ...
  function buildPageModel(detail, statusRow, supervisorDetail, supervisorStatus, supervisorControl, subconsciousPayload, subconsciousDetail, unreadPayload, queueItems, allStatusRows) {
    ...
    const unreadRows = normalizeObjectArray(unreadPayload?.messages);
  ...
  async function saveDetailIdentity() {
    ...
    if (!input) return;
    const saveEditVersion = beginDetailSave();
    ...
    try {
      ...
      await refreshAfterDetailSave(saveEditVersion);
    } catch (e) {
  ...
  let taskListInFlight = false;
  let taskListRefreshQueued = false;
  const taskActionInFlight = new Set();
  const pendingTaskStatusById = new Map();

  function taskActionKey(id, action) { return String(id) + '::' + action; }

  function isTaskActionInFlight(id, action) {
    return taskActionInFlight.has(taskActionKey(id, action));
  }

  function setTaskActionInFlight(id, action, inFlight) {
    const key = taskActionKey(id, action);
    if (inFlight) taskActionInFlight.add(key);
    else taskActionInFlight.delete(key);
  }

  function currentTaskCommentDraft(id) {
    const input = document.getElementById('task-comment-input');
    return input && taskDetailViewId === id ? input.value : '';
  }
  ...
  function normalizeTaskPayload(payload) {
    return normalizeObjectArray(payload).map((item) => {
      ...
      return {
        ...
        comments: normalizeObjectArray(item.comments),
        created_at: item.created_at || item.createdAt || '',
        waiting_reason: typeof item.waiting_reason === 'string' ? item.waiting_reason : '',
        waiting_until: typeof item.waiting_until === 'string' ? item.waiting_until : '',
      };
    }).filter(Boolean);
  }

  function taskListUrl(filterVal) {
    const p = new URLSearchParams();
    p.set('limit', String(TASK_LIST_LIMIT));
    if (filterVal) p.set('assignee', filterVal);
    return '/api/tasks?' + p.toString();
  }

  function currentTaskListFilterValue() {
    const filterEl = document.getElementById('task-filter-assignee');
    if (!filterEl) return '';
    return filterEl.value
      || new URL(window.location).searchParams.get('assignee')
      || storageGet(taskFilterStorage, 'task_filter_assignee')
      || '';
  }

  function taskListFilterKey(filterVal) {
    return String(filterVal || '');
  }

  function finishTaskListRefresh() {
    taskListInFlight = false;
    if (taskListRefreshQueued) {
      taskListRefreshQueued = false;
      taskListRefresh();
    }
  }
  ...
  async function taskListRefresh() {
    if (taskListInFlight) { taskListRefreshQueued = true; return; }
    taskListInFlight = true;
    const root = document.getElementById('task-list-root');
    if (!root) { finishTaskListRefresh(); return; }
    const filterEl = document.getElementById('task-filter-assignee');
    ...
    if (filterEl && !filterEl._initialized) {
      ...
      const saved = urlAssignee || storageGet(taskFilterStorage, 'task_filter_assignee');
      if (saved) filterEl.value = saved;
      filterEl.addEventListener('change', () => {
        filterEl._userOverride = true;
        storageSet(taskFilterStorage, 'task_filter_assignee', filterEl.value);
        const u = new URL(window.location);
    ...
    let filterVal = currentTaskListFilterValue();
    const requestFilterKey = taskListFilterKey(filterVal);
    try {
      const r = await fetch(taskListUrl(filterVal));
      if (!r.ok) throw new Error('status ' + r.status);
      const nextTasks = normalizeTaskPayload(await r.json());
      if (requestFilterKey !== taskListFilterKey(currentTaskListFilterValue())) {
        taskListRefreshQueued = true;
        finishTaskListRefresh();
        return;
      }
      taskListCache = nextTasks;
    } catch (e) {
      if (requestFilterKey !== taskListFilterKey(currentTaskListFilterValue())) {
        taskListRefreshQueued = true;
        finishTaskListRefresh();
    ...
    if (filterEl && !filterEl._populated) {
      ...
      try {
        ...
        if (ar.ok) {
          const agents = normalizeObjectArray(await ar.json());
          const names = new Set();
    ...
    if (taskDetailViewId) {
      ...
      if (found) { renderTaskDetail(found); finishTaskListRefresh(); return; }
  ...
  function renderTaskDetail(task) {
    ...
    const commentDraft = currentTaskCommentDraft(task.id);
    const selectedStatus = pendingTaskStatusById.get(task.id) || task.status;
    const statusDisabled = isTaskActionInFlight(task.id, 'status') ? ' disabled' : '';
    const commentDisabled = isTaskActionInFlight(task.id, 'comment') ? ' disabled' : '';
    const deleteDisabled = isTaskActionInFlight(task.id, 'delete') ? ' disabled' : '';
    let html = '<span class="task-detail-back" onclick="taskBackToList()">&#8592; Back to list</span>'
      ...
      + '</div>'
      + '<div class="task-detail-meta">'
      + '<strong>Status:</strong> <select class="task-status-select" id="task-detail-status" onchange="taskChangeStatus(\\'' + esc(task.id) + '\\')"' + statusDisabled + '>';
    for (const s of statusOptions) {
      html += '<option value="' + s + '"' + (selectedStatus === s ? ' selected' : '') + '>' + s + '</option>';
    }
    html += '</select></div>';
    if (task.description) {
    ...
    html += '<div class="task-comment-form">'
      + '<textarea id="task-comment-input" class="detail-textarea" placeholder="Add a comment..."' + commentDisabled + '></textarea>'
      + '<button class="detail-save" id="task-comment-post" onclick="taskAddComment(\\'' + esc(task.id) + '\\')"' + commentDisabled + '>Post</button>'
      + '</div></div>';
    // Delete button
    html += '<div class="detail-actions" style="margin-top:14px">'
      + '<button class="detail-save" id="task-delete-btn" style="background:rgba(255,100,100,0.1);border-color:rgba(255,100,100,0.3);color:rgba(255,140,140,0.9)" onclick="taskDelete(\\'' + esc(task.id) + '\\')"' + deleteDisabled + '>Delete Task</button>'
      + '</div>';
    root.innerHTML = html;
    const nextInput = document.getElementById('task-comment-input');
    if (nextInput) nextInput.value = commentDraft;
  }
  ...
  async function taskChangeStatus(id) {
    ...
    if (!sel) return;
    const action = 'status';
    if (isTaskActionInFlight(id, action)) return;
    setTaskActionInFlight(id, action, true);
    pendingTaskStatusById.set(id, sel.value);
    sel.disabled = true;
    try {
      ...
      pendingTaskStatusById.delete(id);
      setTaskActionInFlight(id, action, false);
      ...
      pendingTaskStatusById.delete(id);
      setTaskActionInFlight(id, action, false);
      const cur = document.getElementById('task-detail-status');
  ...
  async function taskAddComment(id) {
    ...
    setTaskActionInFlight(id, action, true);
    input.disabled = true;
    const button = document.getElementById('task-comment-post');
    ...
    try {
      ...
      input.value = '';
      const liveInput = document.getElementById('task-comment-input');
      if (taskDetailViewId === id && liveInput) liveInput.value = '';
      setTaskActionInFlight(id, action, false);
      await taskListRefresh();
    } catch (e) {
      ...
      setTaskActionInFlight(id, action, false);
      const curInput = document.getElementById('task-comment-input');
      const curButton = document.getElementById('task-comment-post');
      if (curInput) curInput.disabled = false;
      if (curButton) curButton.disabled = false;
    }
  }
  ...
  async function taskDelete(id) {
    ...
    setTaskActionInFlight(id, action, true);
    const button = document.getElementById('task-delete-btn');
    if (button) button.disabled = true;
    try {
      ...
      setTaskActionInFlight(id, action, false);
      const curButton = document.getElementById('task-delete-btn');
      if (curButton) curButton.disabled = false;
    }
  }
  ...
  function normalizeRefreshOptions(options = false) {
    if (options && typeof options === 'object') {
      return {
        forceDetailRender: options.forceDetailRender === true,
        saveEditVersion: Number.isFinite(options.saveEditVersion) ? options.saveEditVersion : null,
      };
    }
    return { forceDetailRender: options === true, saveEditVersion: null };
  }

  function mergeRefreshOptions(previous, next) {
    if (!previous) return next;
    return {
      forceDetailRender: previous.forceDetailRender === true || next.forceDetailRender === true,
      saveEditVersion: next.saveEditVersion !== null ? next.saveEditVersion : previous.saveEditVersion,
    };
  }
  ...
  async function fetchOptionalJson(url, fallback) {
    try {
      ...
      return { response: { ok: false }, payload: fallback };
    }
  }

  async function refresh(options = false) {
    const refreshOptions = normalizeRefreshOptions(options);
    const forceDetailRender = refreshOptions.forceDetailRender;
    const saveEditVersion = refreshOptions.saveEditVersion;
    if (refreshInFlight) {
      refreshQueued = mergeRefreshOptions(refreshQueued, refreshOptions);
      return false;
    }
    refreshInFlight = true;
    try {
      const [statusResult, detailResult, controlResult, subconsciousResult, subconsciousDetailResult, agentDetailResult, agentStatusResult, unreadResult, queueResult, presetsResult] = await Promise.all([
        fetchRequiredJson('/api/supervisor/status'),
        fetchRequiredJson('/api/supervisor/agents/' + encodeURIComponent(agent) + '?limit=180'),
        fetchOptionalJson('/api/supervisor/control', {}),
        fetchOptionalJson('/api/subconscious/events/' + encodeURIComponent(agent) + '?limit=40', { ok: false, events: [] }),
        fetchOptionalJson('/api/subconscious/detail/' + encodeURIComponent(agent), { ok: false, stage: 'unknown' }),
        fetchOptionalJson('/api/agents/detail/' + encodeURIComponent(agent), { error: 'agent detail unavailable' }),
        fetchOptionalJson('/api/agents/status', []),
        fetchOptionalJson('/api/agents/' + encodeURIComponent(agent) + '/unread-messages?limit=40', { unread_total: 0, messages: [] }),
        fetchOptionalJson('/api/queue', []),
        fetchOptionalJson('/api/framework-presets', []),
      ]);
      const statusRes = statusResult.response;
      const detailRes = detailResult.response;
      const statusPayload = statusResult.payload;
      const detail = detailResult.payload;
      const supervisorControlPayload = controlResult.payload;
      const subconsciousPayload = subconsciousResult.payload;
      const subconsciousDetailPayload = subconsciousDetailResult.payload;
      const agentDetailPayload = agentDetailResult.payload;
      const agentStatusPayload = agentStatusResult.payload;
      const unreadPayload = unreadResult.payload;
      const queuePayload = queueResult.payload;
      const presetsPayload = presetsResult.payload;
      _presetCache = normalizePresetPayload(presetsPayload);
      if (!statusRes.ok || !detailRes.ok) throw new Error((detail && detail.error) || 'load failed');
      const statusRows = normalizeAgentStatusPayload(agentStatusPayload);
      ...
      const hasDirtyDetailChanges = hasUnsavedDetailChanges(agentDetailPayload, supervisorControlPayload, subconsciousDetailPayload);
      const shouldPreserveDirty = shouldPreserveDetailSettings({
        forceDetailRender,
        hasUnsavedChanges: hasDirtyDetailChanges,
        saveEditVersion,
        currentEditVersion: detailEditVersion,
      });
      ...
      if (refreshQueued) {
        const queuedOptions = refreshQueued;
        refreshQueued = null;
        refresh(queuedOptions);
      }
    }
    return true;
  }

  function scheduleAgentDetailRefresh(delayMs) {
    if (refreshTimer) clearTimeout(refreshTimer);
    const nextDelay = typeof delayMs === 'number'
      ? delayMs
      : (document.hidden ? AGENT_DETAIL_REFRESH_HIDDEN_MS : AGENT_DETAIL_REFRESH_VISIBLE_MS);
    refreshTimer = setTimeout(runAgentDetailRefreshLoop, nextDelay);
  }

  async function runAgentDetailRefreshLoop() {
    await refresh(false);
    scheduleAgentDetailRefresh();
  }
  ...
  window.addEventListener('visibilitychange', () => {
    scheduleAgentDetailRefresh(document.hidden ? AGENT_DETAIL_REFRESH_HIDDEN_MS : 0);
  });
  ...
  syncStickyOffsets();
  refresh().finally(() => scheduleAgentDetailRefresh());
})();
</script>
</body>
</html>`;
