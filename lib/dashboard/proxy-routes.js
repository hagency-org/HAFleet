...
export function installProjectBoardProxyRoutes(app, { backendBaseUrl, backendFetch }) {
  app.get('/api/project-board', async (req, res) => {
    try {
      const url = new URL(`${backendBaseUrl}/api/project-board`);
      if (typeof req.query.activity_limit === 'string' && req.query.activity_limit.trim()) {
        url.searchParams.set('activity_limit', req.query.activity_limit.trim());
      }
      const response = await backendFetch(url);
      const data = await response.json().catch(() => ({ error: `backend status ${response.status}` }));
