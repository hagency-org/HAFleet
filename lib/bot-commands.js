...
export default class BotCommands {
  ...
  async cmdDm(roomId, args, humanName) {
    ...
    // Create or join existing DM room via bridge
    const dmRoomId = await this.bridge.ensureDmRoom(agentName, humanName);
    if (dmRoomId) {
      // Check if human is already in the room
      let alreadyIn = false;
      try {
        const members = await this.botClient.getJoinedRoomMembers(dmRoomId);
        const humanUserId = `@${humanName}:matrix.kusuri.ai`;
        alreadyIn = members.includes(humanUserId);
      } catch {}
      ...
      if (alreadyIn) {
        ...
        await this.reply(roomId,
          `DM room ready for ${agentName} (you've been added)`,
          `DM room ready for <b>${escHtml(agentName)}</b> — you've been added`
        );
      }
    } else {
