...
function getSessionActivitySec(session) {
  try {
    const out = execSync(`tmux display-message -p -t ${JSON.stringify(session)} "#{session_activity}"`, { timeout: 3000 })
      .toString().trim();
...
export default class BotCommands {
  ...
  async sendAgentStatusSnapshot(roomId, agentName, target, tailLines = 5, prefix = '') {
    let tail = '';
    try {
      tail = execSync(`tmux capture-pane -t ${JSON.stringify(target)} -p -S -${tailLines}`, { timeout: 5000 }).toString();
    } catch (e) {
  ...
  scheduleAutoStatusAfterControl(roomId, agentName, target, session, baseActivitySec) {
    ...
    setTimeout(async () => {
      ...
      const curActivitySec = getSessionActivitySec(session);
      if (!curActivitySec) {
        await this.sendAgentStatusSnapshot(
          ...
          target,
          5,
          '[auto-status] Could not determine tmux activity; sending status snapshot.'
        );
        return;
      }

      const idleSec = Math.max(0, nowSec - curActivitySec);
      const activeSpanSec = (baseActivitySec && curActivitySec > baseActivitySec)
        ? (curActivitySec - baseActivitySec)
        : 0;
      const stillIdle = curActivitySec <= (baseActivitySec || 0);
      const shortActive = activeSpanSec > 0 && activeSpanSec < MIN_ACTIVE_SEC;
      const shouldAutoStatus = stillIdle || shortActive;
      if (!shouldAutoStatus) return;

      const reason = stillIdle
        ? `still idle after 5s (idle=${idleSec}s)`
        : `active duration < ${MIN_ACTIVE_SEC}s (active=${activeSpanSec}s, idle=${idleSec}s)`;
      await this.sendAgentStatusSnapshot(
        roomId,
        agentName,
        target,
  ...
  async cmdAgentctl(roomId, args, context = {}, shortMode = false) {
    ...
    if (action === 'send') {
      ...
      const baseActivitySec = getSessionActivitySec(session);
