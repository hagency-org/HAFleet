...
export default class BotCommands {
  ...
  async handle(roomId, senderId, text, context = {}) {
    ...
    try {
      switch (command) {
        ...
        case '!autoregister': return await this.cmdAutoregister(roomId);
        case '!agent':    return await this.cmdAgent(roomId, args);
        ...
        case '!identity': return await this.cmdIdentity(roomId, args);
  ...
  async cmdHelp(roomId) {
    const plain = [
      ...
      '  !autoregister    — Auto-register/deregister agents',
      ...
      '  !identity <agent> <text>       — Set agent identity',
    ...
    const html = [
      ...
      '<code>!autoregister</code> — Auto-register/deregister agents<br>',
      '<code>!bridge</code> — Bridge internal state<br>',
      '<br><b>Detail:</b><br>',
      '<code>!agent &lt;name&gt;</code> — Agent details<br>',
      '<code>!group &lt;name&gt;</code> — Group details<br>',
      ...
      '<code>!identity &lt;agent&gt; &lt;text&gt;</code> — Set agent identity<br>',
  ...
  async cmdStatus(roomId) {
    ...
    const html = [
      ...
      `Tmux sessions: <b>${sessionCount}</b><br>`,
      `Bridge: <b>running</b>`,
    ].join('');
  ...
  async cmdAgents(roomId) {
    const agents = await api('GET', '/api/agents');
    if (!agents.length) {
      await this.reply(roomId, 'No agents registered.');
      return;
    }
    ...
    try {
      ...
      if (out) sessions = new Set(out.split('\n'));
    } catch { /* no tmux */ }

    const lines = ['=== Registered Agents ==='];
    const htmlLines = ['<b>Registered Agents</b><br><br>'];

    for (const a of agents) {
      const alive = sessions.has(a.name);
      const icon = alive ? '●' : '○';
      const idText = a.identity ? ` — ${a.identity}` : '';
      const line = `${icon} ${a.name}${idText}`;
      lines.push(line);
      const color = alive ? '#69db7c' : '#888';
  ...
  async cmdMcp(roomId) {
    ...
    const htmlLines = ['<b>MCP Status</b><br><br><table>',
      '<tr><th>Session</th><th>MCP</th><th>Client</th><th>Registered</th></tr>'];

    for (const sess of allSessions) {
      const info = mcpSessions[sess];
      const hasMcp = info ? 'YES' : 'no';
      const client = info ? info.client : '-';
      const isReg = registered.has(sess) ? 'YES' : 'no';

      lines.push(`  ${sess.padEnd(25)} ${hasMcp.padEnd(8)} ${client.padEnd(8)} ${isReg}`);

      const mcpColor = info ? '#69db7c' : '#888';
      const regColor = registered.has(sess) ? '#69db7c' : '#888';
      htmlLines.push(
        `<tr><td><code>${escHtml(sess)}</code></td>` +
        `<td style="color:${mcpColor}">${hasMcp}</td>` +
        `<td>${escHtml(client)}</td>` +
        `<td style="color:${regColor}">${isReg}</td></tr>`
      );
    }
  ...
  async cmdAutoregister(roomId) {
    const results = { registered: [], deregistered: [], skipped: [], errors: [] };
    ...
    // 2. Detect MCP sessions (reuse check-mcp logic)
    ...
    const agents = await api('GET', '/api/agents');
    const registeredMap = {};
    for (const a of agents) registeredMap[a.name] = a;

    // 4. Auto-register: has MCP + not registered
    for (const sess of mcpSessions) {
      if (registeredMap[sess]) {
        results.skipped.push(sess);
        continue;
      }
      try {
        await api('POST', '/api/agents', { name: sess, tmux: `${sess}:0.0`, type: 'agent' });
        results.registered.push(sess);
      } catch (e) {
        results.errors.push(`register ${sess}: ${e.message}`);
      }
    }

    // 5. Auto-deregister: registered agent but session doesn't exist
    for (const a of agents) {
      if (a.type === 'human') continue; // never touch humans
      if (!sessionSet.has(a.name)) {
        try {
          await api('DELETE', `/api/agents/${encodeURIComponent(a.name)}`);
          results.deregistered.push(a.name);
        } catch (e) {
          results.errors.push(`deregister ${a.name}: ${e.message}`);
        }
      }
    }

    // 6. Report
    const lines = ['=== Autoregister Results ==='];
    if (results.registered.length)
      lines.push(`Registered (${results.registered.length}): ${results.registered.join(', ')}`);
    if (results.deregistered.length)
      lines.push(`Deregistered (${results.deregistered.length}): ${results.deregistered.join(', ')}`);
    if (results.skipped.length)
      lines.push(`Already registered (${results.skipped.length}): ${results.skipped.join(', ')}`);
    if (results.errors.length)
      lines.push(`Errors: ${results.errors.join('; ')}`);
    if (!results.registered.length && !results.deregistered.length)
      lines.push('No changes needed.');

    lines.push(`\nMCP sessions: ${mcpSessions.size} / ${allSessions.length} total`);

    const htmlLines = ['<b>Autoregister Results</b><br><br>'];
    if (results.registered.length)
      htmlLines.push(`<span style="color:#69db7c">Registered (${results.registered.length}):</span> ${results.registered.map(s => '<code>' + escHtml(s) + '</code>').join(', ')}<br>`);
    if (results.deregistered.length)
      htmlLines.push(`<span style="color:#ffd43b">Deregistered (${results.deregistered.length}):</span> ${results.deregistered.map(s => '<code>' + escHtml(s) + '</code>').join(', ')}<br>`);
    if (results.skipped.length)
      htmlLines.push(`<span style="color:#888">Already registered (${results.skipped.length}):</span> ${results.skipped.map(s => escHtml(s)).join(', ')}<br>`);
    if (results.errors.length)
      htmlLines.push(`<span style="color:#ff6b6b">Errors:</span> ${results.errors.map(e => escHtml(e)).join('; ')}<br>`);
    if (!results.registered.length && !results.deregistered.length)
      htmlLines.push('No changes needed.<br>');

    htmlLines.push(`<br>MCP sessions: <b>${mcpSessions.size}</b> / ${allSessions.length} total`);

    await this.reply(roomId, lines.join('\n'), htmlLines.join(''));
  }

  async cmdAgent(roomId, args) {
    if (!args.length) {
      await this.reply(roomId, 'Usage: !agent <name>');
      return;
    }
    const name = args[0];
    ...
    if (matrixRoomId) {
      ...
      try {
        ...
        for (const userId of memberIds) {
          ...
          // Check if this is an agent (ac_ prefix) — use their own token to leave
          const agentMatch = userId.match(/^@ac_([^:]+):/);
          if (agentMatch) {
            const agentToken = this.bridge.getAgentToken?.(agentMatch[1]);
            if (agentToken) {
              try {
                await fetch(`${HOMESERVER}/_matrix/client/v3/rooms/${roomEnc}/leave`, {
                  method: 'POST',
          ...
          // Human or unknown — try kick, ignore if no permission
          try {
            await fetch(`${HOMESERVER}/_matrix/client/v3/rooms/${roomEnc}/kick`, {
              method: 'POST',
  ...
  async cmdDm(roomId, args, humanName) {
    ...
    // Create DM room via bridge
    const dmRoomId = await this.bridge.ensureDmRoom(agentName, humanName);
    if (dmRoomId) {
      await this.reply(roomId,
        `DM room created for ${agentName} ↔ ${humanName}`,
        `DM room created for <b>${escHtml(agentName)}</b> ↔ <b>${escHtml(humanName)}</b>`
      );
    } else {
  ...
  async cmdIdentity(roomId, args) {
    if (args.length < 2) {
      await this.reply(roomId, 'Usage: !identity <agent> <identity text...>');
      return;
    }
    const agentName = args[0];
    const identity = args.slice(1).join(' ');
