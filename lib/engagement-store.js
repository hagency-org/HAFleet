...
function posInt(value, field) {
  ...
  if (!Number.isFinite(n) || n <= 0) throw new EngagementError('bad_request', `${field} must be a positive number`);
  return Math.floor(n);
}
...
export function routeRequest({ request, whitelisted, offer, remainingTokens, activeForRole = 0 }) {
  ...
  if (offer.rateCap && request.ratePerDay && request.ratePerDay > offer.rateCap) {
    return { route: 'overOffer', autoJoin: false };
  }
  /*
...
export function createEngagementStore({ load, persist, now = () => Date.now() } = {}) {
  ...
  function record(type, detail) {
    ...
    // still has history.
    while (audit.length > AUDIT_LIMIT) audit.shift();
  }
  ...
  return {
    ...
    addToWhitelist({ projectRoomId, displayName, addedBy }) {
      ...
      record(existed ? 'whitelist.updated' : 'whitelist.added', { projectRoomId: room, by });
      commit(() => {
    ...
    removeFromWhitelist({ projectRoomId, by }) {
      ...
      const stillActive = Object.values(engagements)
        ...
        .map((e) => e.id);
      record('whitelist.removed', { projectRoomId: room, by: by ?? 'operator', stillActive: stillActive.length });
      commit(() => { whitelist[room] = previous; audit.pop(); }, 'whitelist');
    ...
    setOffer({ role, count, budgetCapPerEngagement, rateCap, published, by }) {
      ...
      offers[key] = next;
      record('offer.set', { role: key, published: next.published, by: next.updatedBy });
      commit(() => {
    ...
    createRequest({ project, projectRoomId, role, requester, requestedTokens, ratePerDay, agent, offer, remainingTokens }) {
      ...
      record(decision.autoJoin ? 'engagement.auto_joined' : 'engagement.pending', {
        engagementId: engagement.id, project: engagement.project, role: engagement.role, route: decision.route,
      });
      commit(() => { delete engagements[engagement.id]; audit.pop(); }, 'engagement');
      return engagement;
    },
    ...
    decide({ engagementId, approve, allocatedTokens, remainingTokens, by, reason }) {
      ...
      if (!approve) {
        ...
        record('engagement.rejected', { engagementId: e.id, by: e.decidedBy });
        commit(() => { engagements[e.id] = snapshot; audit.pop(); }, 'verdict');
      ...
      e.decidedAt = now();
      e.decidedBy = by ?? 'operator';
      record('engagement.approved', { engagementId: e.id, allocatedTokens: alloc, by: e.decidedBy });
    ...
    setBindingOutcome(engagementId, { bound, ownerMxid = null, error = null }) {
      ...
      record(bound ? 'engagement.bound' : 'engagement.bind_failed', {
        engagementId, ownerMxid, error: e.bindError,
      });
      commit(() => { engagements[engagementId] = snapshot; audit.pop(); }, 'binding outcome');
      return e;
    },
    ...
    revoke({ engagementId, by, reason }) {
      ...
      record('engagement.revoked', { engagementId: e.id, by: by ?? 'operator' });
      commit(() => { engagements[e.id] = snap; audit.pop(); }, 'revocation');
      return engagements[e.id];
    },
