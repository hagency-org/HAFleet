...
export async function ensureRepresentative({
  ...
  if (credential.kind === 'appservice') {
     ...
     *
     * THE MASQUERADE IS LOAD-BEARING, NOT MERELY A VALIDATION STYLE — do not "simplify" this to a
     * bare `as_token` whoami. Verified against the Palpo 0.4.0 build this deployment runs: nothing
     * there creates the `sender_localpart` account, because `register_appservice` only inserts the
     * registration row. Palpo's NON-masqueraded auth branch then does a bare lookup for a user
     * carrying that `appservice_id` and raises a database NotFound until one exists. A masqueraded
     * call runs `get_or_create_appservice_user` instead, so THIS call is what bootstraps the
     * representative's account — and `is_user_match` permits `sender_localpart` by definition, so it
     * is always allowed to be the first one.
     *
     * That ordering is inverted from Synapse and Conduit, where the sender user is provisioned at
     * registration time and a bare call works immediately. Code that is correct against those two
     * fails against Palpo on the very first request, which is the kind of difference that reads as
     * "our credential is broken".
     */
