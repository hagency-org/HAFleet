...
function isMcpAliveByPidFile(agentName) {
  try {
    const pid = parseInt(readFileSync(mcpPidFilePath(agentName), 'utf-8').trim(), 10);
    if (!pid || !Number.isFinite(pid)) return false;
    process.kill(pid, 0); // signal 0 = existence check
    // Verify process identity via /proc to guard against PID reuse
    const cmdline = readFileSync(`/proc/${pid}/cmdline`, 'utf-8');
    if (!cmdline.includes('mcp-server')) return false;
    return true;
  } catch {
    return false;
  }
