...
export function createSupervisorSnapshotStore({ initialData, save }) {
  ...
  function restoreState(snapshot) {
    ...
    Object.assign(targets, snapshot.targets);
    events.splice(0, events.length, ...snapshot.events);
    for (const key of Object.keys(leases)) {
      delete leases[key];
    }
    Object.assign(leases, snapshot.leases);
    globalEnabled = snapshot.globalEnabled;
    globalDisabledReason = snapshot.globalDisabledReason;
  }
  ...
  function persist() {
    ...
    try {
      ...
      throw snapshotError('snapshot_persistence_failed', 'supervisor snapshot persistence failed');
    }
    if (ok === false) {
      throw snapshotError('snapshot_persistence_failed', 'supervisor snapshot persistence failed');
    }
  ...
  function updateAssessment(targetAgent, supervisorAgent, assessment) {
    ...
    const rawConfidence = typeof assessment.confidence === 'number'
  ...
  function removeTarget(targetAgent) {
    const had = targetAgent in targets;
    if (!had) return false;
    return commitMutation(() => {
      delete leases[targetAgent];
      delete targets[targetAgent];
      // Remove events for this target
      for (let i = events.length - 1; i >= 0; i--) {
        if (events[i].agent === targetAgent) events.splice(i, 1);
      }
      return { value: true };
    });
  }
