...
function publicSide(record) {
  ...
  return {
     ...
     * A Palpo registration only loads on restart (verified against 0.4.0), so between issuing and the
     * project side installing it there is a wait we do not control, and a wait with no age reads as a
     * hang. `hasCredential && accessState === 'unverified'` IS that state; this makes it legible.
     */
    credentialIssuedAt: record.credentialIssuedAt ?? null,
...
export class ProjectSideStore {
  ...
  setCredential(idValue, value) {
     ...
     */
    record.credentialIssuedAt = record.credential ? this.now() : null;
    record.updatedAt = this.now();
