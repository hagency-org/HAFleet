...
export class ProjectSideStore {
  ...
  /**
   * Deactivate a side without forgetting it.
   *
   * The reversible half of decision 7. Deactivation stops a side being used for new work while
   * leaving the credential in place, which is what makes an ordered cascade possible at all: the
   * caller can close a side to new engagements, then do the work that still needs the credential —
   * leaving rooms, revoking tokens — and only then call `removeSide`.
   */
