...
export function resolveAgentTokenMode(env = process.env) {
  const configuredMode = (env.AGENTCHAT_AGENT_TOKEN_MODE || 'audit').trim().toLowerCase() || 'audit';
...
export function buildServerCredentialReadiness({ env = process.env } = {}) {
  ...
  const serverTokenConfigured = Boolean(normalizeOptionalText(env.AGENTCHAT_SERVER_TOKEN, 512));
  return {
...
export function authorizeSubconsciousEventIngest(req, {
  ...
  const expectedToken = normalizeOptionalText(env.AGENTCHAT_SUBCONSCIOUS_EVENT_TOKEN, 512);
  if (!expectedToken) {
    return {
      ok: false,
      status: 403,
