...
import { isLocalAgentServer, normalizeServer, resolveLocalServerId } from './server-identity.js';

export { isLocalAgentServer, normalizeServer, resolveLocalServerId };
...
export function assertRuntimeDir(runtimeRoot) {
  ...
  // Check for stale-backup siblings of the data dir (scan even if data/ doesn't exist yet)
  try {
    const siblings = readdirSync(runtimeRoot);
    const stale = siblings.find(s => /^data[._-]stale/i.test(s));
    if (stale) {
      throw new Error(`FATAL: stale data marker '${path.join(runtimeRoot, stale)}' found — runtime dir may be poisoned`);
    }
  } catch (e) {
