...
export function agentExpectsMcp(agent) {
  ...
  // octos loads the agent-chat MCP server as a client, but agent-chat's
  // claude-oriented MCP-presence probe cannot detect it — suppress the false
  // 'degraded' badge the same way codex does.
  if (agent.type === 'octos') return false;
