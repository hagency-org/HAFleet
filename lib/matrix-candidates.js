...
export async function describeMatrixReach({
   ...
   */
  const syncSide = String(env.HAFLEET_APPSERVICE_SYNC_SIDE ?? '').trim();
  const syncUrl = String(env.HAFLEET_APPSERVICE_SYNC_URL ?? '').trim();
  const sync = Boolean(syncSide && syncUrl);
