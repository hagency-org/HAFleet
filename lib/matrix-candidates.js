 ...
 */
export function diagnoseEdgeInbound(status) {
  ...
  if (transactions === 0) {
    return {
      state: 'never-called',
      detail: 'the homeserver has never called this edge. Either nothing has happened in a room it can see '
    ...
    appservice: port
      ...
      : edge
        ...
        : {
          ...
          port: null,
          inboundVia: null,
          reason: 'HAFLEET_APPSERVICE_PORT is not set and no co-located edge is configured, so nothing '
            + 'will receive your homeserver\'s events. Either set the port, or run '
            + 'bin/hafleet-appservice-edge beside the homeserver and set HAFLEET_EDGE_URL.',
          callbackCandidates: [],
        },
  };
}
