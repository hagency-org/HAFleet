...
export function validateLaunchExtraArgs(framework, value) {
  ...
  for (let index = 0; index < tokens.length; index += 1) {
    ...
    if (token === '--') {
      return { ok: false, reason: 'the launch argument delimiter is managed by agent-chat' };
    }
    if ((normalizedFramework === 'claude' || !normalizedFramework) && isClaudePolicyOverride(token)) {
      return { ok: false, reason: `Claude permission policy flag is managed by agent-chat: ${token}` };
    }
    if ((normalizedFramework === 'codex' || !normalizedFramework) && isCodexPolicyOverride(token)) {
      return { ok: false, reason: `Codex Level 2 policy flag is managed by agent-chat: ${token}` };
    }
    if ((normalizedFramework === 'codex' || !normalizedFramework)
      ...
      if (isCodexConfigPolicyOverride(configValue)) {
        return { ok: false, reason: `Codex Level 2 config is managed by agent-chat: ${configValue.split('=')[0]}` };
      }
    }
    if ((normalizedFramework === 'codex' || !normalizedFramework)
      && token.startsWith('--config=')
      && isCodexConfigPolicyOverride(token.slice('--config='.length))) {
      return { ok: false, reason: 'Codex Level 2 config is managed by agent-chat' };
    }
    if ((normalizedFramework === 'codex' || !normalizedFramework)
      && token.startsWith('-c=')
