...
const HAFLEET_SOURCE = 'claude-subconscious-v1';
...
function deterministicLettaAgentId(seed) {
  const hex = hashHex(seed || 'hafleet-subconscious').slice(0, 32).padEnd(32, '0');
  return `agent-${hex.slice(0, 8)}-${hex.slice(8, 12)}-${hex.slice(12, 16)}-${hex.slice(16, 20)}-${hex.slice(20, 32)}`;
}

function appendLog(line) {
  const stateDir = normalizePath(process.env.HAFLEET_AGENT_STATE_DIR);
  if (!stateDir) return;
...
function resolveAgentName(input) {
  return (
    ...
    || normalizeText(process.env.HAFLEET_AGENT_ID, 128)
...
function resolveLettaState(agentName) {
  const stateDir = normalizePath(process.env.HAFLEET_AGENT_STATE_DIR);
  const resolvedPath = normalizePath(process.env.HAFLEET_LETTA_STATE_FILE)
  ...
  if (!resolvedPath) {
    return {
      ...
      source: 'generated',
      guidance: null,
      enabled: process.env.HAFLEET_SUBCONSCIOUS_ENABLED !== '0',
    };
  }
  ...
  if (lettaAgentId) {
    ...
    const stableSeed = `${normalizeText(process.env.HAFLEET_AGENT_ID, 256) || agentName}:${agentName}`;
    lettaAgentId = deterministicLettaAgentId(stableSeed);
  }

  const enabled = process.env.HAFLEET_SUBCONSCIOUS_ENABLED !== '0';
...
async function postSubconsciousEvent(payload) {
  const url = normalizeText(process.env.HAFLEET_SUBCONSCIOUS_EVENT_URL, 2048);
  if (!url) return;

  const headers = { 'Content-Type': 'application/json' };
  const token = normalizeText(process.env.HAFLEET_SUBCONSCIOUS_EVENT_TOKEN, 512);
...
function resolveInvokeUrl() {
  return normalizeText(process.env.HAFLEET_SUBCONSCIOUS_INVOKE_URL, 2048);
}

function resolveStopUrl() {
  const explicit = normalizeText(process.env.HAFLEET_SUBCONSCIOUS_STOP_URL, 2048);
  if (explicit) return explicit;
  const eventUrl = normalizeText(process.env.HAFLEET_SUBCONSCIOUS_EVENT_URL, 2048);
  if (!eventUrl) return null;
  return eventUrl.replace(/\/api\/subconscious\/events\/?$/i, '/api/subconscious/upstream/stop');
}

function resolveUserPromptUrl() {
  const explicit = normalizeText(process.env.HAFLEET_SUBCONSCIOUS_USER_PROMPT_URL, 2048);
  if (explicit) return explicit;
  const eventUrl = normalizeText(process.env.HAFLEET_SUBCONSCIOUS_EVENT_URL, 2048);
  if (!eventUrl) return null;
...
function resolvePreToolUrl() {
  const explicit = normalizeText(process.env.HAFLEET_SUBCONSCIOUS_PRETOOL_URL, 2048);
  if (explicit) return explicit;
  const eventUrl = normalizeText(process.env.HAFLEET_SUBCONSCIOUS_EVENT_URL, 2048);
  if (!eventUrl) return null;
...
function emitGuidanceAdditionalContext(hookName, agentName, guidance) {
  ...
  const context = `<hafleet_subconscious agent="${escapeXml(agentName)}">${escapeXml(clean)}</hafleet_subconscious>`;
  const payload = {
    hookSpecificOutput: {
