...
REPO_DIR="${HAFLEET_DEPLOY_DIR:?HAFLEET_DEPLOY_DIR must be set}"
DEPLOY_BRANCH="${HAFLEET_DEPLOY_BRANCH:-master}"
POLL_SEC="${HAFLEET_POLL_SEC:-30}"
DEPLOY_SERVICES="${HAFLEET_DEPLOY_SERVICES:-hafleet-dev-backend.service hafleet-dev-web.service}"
...
init_deploy_state() {
  if [ -z "$DEPLOY_STATE_DIR" ]; then
    DEPLOY_STATE_DIR="$(run_git rev-parse --absolute-git-dir)/hafleet-dev-autodeploy"
  fi
