  ...
  test('codex_hook_preflight_precedes_tmux_and_requires_exact_trust', async () => {
    ...
    try {
      ...
      for (const file of ['bin/agent-up', 'remote/bin/agent-up']) {
        ...
        // One per framework per resume/fresh path. bin/agent-up carries three
        // frameworks (claude, codex, hermes) so six; remote/bin/agent-up is a
        // separately maintained file — MANAGED_SPECS sources it directly rather
        // than copying from bin/agent-up — and still has only claude and codex.
